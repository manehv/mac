 ...
 * @since 0.0.1
 */
class WTGCSVEXPORTER_createdetailedprofiles_View extends WTGCSVEXPORTER_View {
    ...
    protected $view_name = 'createdetailedprofiles';
    ...
    */
    public function meta_box_array() {
        ...
        $this->meta_boxes_array = array(
            array( $this->view_name . '-tablesdetailed', __( 'A. Database Tables', 'wtgcsvexporter' ), array( $this, 'parent' ), 'normal','default',array( 'formid' => 'tablesdetailed' ), true, 'activate_plugins' ),
        );  

        // if database tables selected - list the columns to be exported
        if( isset( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['tables'] ) ) {
            $this->meta_boxes_array = array_merge( $this->meta_boxes_array, array( array( $this->view_name . '-columnsdetailed', __( 'B. Table Columns', 'wtgcsvexporter' ), array( $this, 'parent' ), 'normal','default',array( 'formid' => 'columnsdetailed' ), true, 'activate_plugins' ) ) );
        }
        ...
        if( isset( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['primary'] ) ) {
            $this->meta_boxes_array = array_merge( $this->meta_boxes_array, array( array( $this->view_name . '-finishdetailed', __( 'Finish', 'wtgcsvexporter' ), array( $this, 'parent' ), 'normal','default',array( 'formid' => 'finishdetailed' ), true, 'activate_plugins' ) ) );
        }
    ...
    */
    public function datatables( $data, $box ) {       
        $WPTableObject = new WTGCSVEXPORTER_createdetailedprofiles_Exampledata();
    ...
    */
    ...
    /**
    * Select tables that contain data that is to be exported.
    * 
    * After selecting applicable tables, lists of columns become available. 
    * 
    * @author Ryan Bayne
    * @package WTG CSV Exporter
    * @since 0.0.1
    * @version 1.0
    */
    public function postbox_createdetailedprofiles_tablesdetailed( $data, $box ) { 
        $this->UI->postbox_content_header( $box['title'], $box['args']['formid'], __( 'Select tables you would like to be involved in the export process. There is no need to select tables that are associated with the primary source. After submitting this form some lists of columns names will become available and you must make further selections.', 'wtgcsvexporter' ), false );        
        ...
        if( isset( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['tables'] ) )
        {
            $current_value = $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['tables'];    
        }
 
        $tables_array = $this->DB->get_tables();
        
        // re-build array so that our keys are not numeric
        $items_array = array();
        foreach( $tables_array as $key => $table_name ){  
            $items_array[ $table_name ] = $table_name;
        }

        echo '<table class="form-table">';
        $this->FORMS->checkboxes_basic( $box['args']['formid'], 'includedtables', 'includedtables', __( 'Include Tables', 'wtgcsvexporter' ), $items_array, $current_value, true, array(), false );        
        echo '</table>';
    ...
    * @version 1.0
    */
    public function postbox_createdetailedprofiles_columnsdetailed( $data, $box ) { 
        ...
        {
            echo '<table class="form-table">';
            foreach( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['tables'] as $key => $table_n ) {
                 
                $current_tables_column = $this->DB->get_tablecolumns( $table_n, true, true );
                 
                // rebuild columns array making keys the column names
                $items_array = array();
                foreach( $current_tables_column as $key => $col_n ) {
                    $items_array[ $col_n ] = $col_n;    
                }
                  
                // set current value
                if( isset( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['columns'][$table_n] ) && is_array( $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['columns'][$table_n] ) )
                {
                    $current_value = $wtgcsvexporter_settings['csvexportprofiles']['singlefileprofile']['columns'][$table_n];    
                } 
                
                $this->FORMS->checkboxes_basic( $box['args']['formid'], 'expcols' . $table_n, 'cols' . $table_n, __( $table_n, 'wtgcsvexporter' ), $items_array, $current_value, false, array(), true );        
            }
            echo '</table>';
        }
    ...
    * @version 1.0
    */
    public function postbox_createdetailedprofiles_finishdetailed( $data, $box ) { 
        ...
        $this->FORMS->input_subline( $this->UI->linkaction( 'wtgcsvexporter_createdetailedprofiles', 'testsinglefileprofile', __( 'Export your data now to test your selections before clicking Save.', 'wtgcsvexporter' ), __( 'Export Test', 'wtgcsvexporter' ) ), __( 'Want to test first?', 'wtgcsvexporter' ) );
...
*/
class WTGCSVEXPORTER_createdetailedprofiles_Exampledata extends WP_List_Table {
