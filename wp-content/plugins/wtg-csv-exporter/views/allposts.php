<?php
/**
 * Export all posts (pages, posts and custom post types) 
 *
 ...
 * @since 0.0.1
 */
class WTGCSVEXPORTER_Allposts_View extends WTGCSVEXPORTER_View {
    ...
    */
    public function meta_box_array() {
        ...
        return $this->meta_boxes_array = array(
            ...
            array( $this->view_name . '-exportallpostscolumns', __( 'Export', 'wtgcsvexporter' ), array( $this, 'parent' ), 'normal','default',array( 'formid' => 'exportallpostscolumns' ), true, 'activate_plugins' ),
        );    
    }
    ...
    */
    public function datatables( $data, $box ) {       
        $WPTableObject = new WTGCSVEXPORTER_Allposts_Example();
    ...
    * @version 1.0
    */
    public function postbox_allposts_exportallpostscolumns( $data, $box ) { 
        global $wpdb, $wtgcsvexporter_settings;
        
        $current_settings_array = array();
        if( isset( $wtgcsvexporter_settings['csvexportprofiles']['allposts']['tables']['posts'] ) ) { 
            $current_settings_array = $wtgcsvexporter_settings['csvexportprofiles']['allposts']['tables']['posts'];
        }
        
        $this->UI->postbox_content_header( $box['title'], $box['args']['formid'], __( 'Select the columns of data you would like included in your .csv file. This form will save your selections so that future exports are quicker and each .csv file is accurate to the one before.', 'wtgcsvexporter' ), false );        
        $this->Forms->form_start( $box['args']['formid'], $box['args']['formid'], $box['title'] );
        ?>  

        <table class="form-table">
        <?php         
        $table_columns = $wpdb->get_col( "DESC " . $wpdb->prefix . 'posts', 0 );
        $items_array = array();
        foreach( $table_columns as $column ) {
            $items_array[ $column ] = $column;        
        }
        $this->Forms->checkboxes_advanced( $box['args']['formid'], 'exportablecolumns', 'exportablecolumns', __( 'Select Data', 'wtgcsvexporter' ), $current_settings_array, $items_array, 1, 100, false, true, true, false, false );
        ?>
        </table>
        
        <?php
        $this->UI->postbox_content_footer( __( 'Export', 'wtgcsvexporter' ) );
    }      
}
...
*/
class WTGCSVEXPORTER_Allposts_Example extends WP_List_Table {
