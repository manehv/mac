...
add_action( 'add_meta_boxes', 'wtgcsvexporter_add_meta_boxes_post' );
add_action( 'save_post', 'wtgcsvexporter_save_meta_boxes_post',10,2 );
/**
* Must be called using add_action( 'init', 'wtgcsvexporter_register_customposttype_contentdesigns' )
* Registers custom post type for content only
*/
function wtgcsvexporter_init_posttype_contentdesigns() {
    $labels = array(
        'name' => _x( 'WTG CSV Exporter Content Templates', 'post type general name' ),
        'singular_name' => _x( 'WTG CSV Exporter Content Template', 'post type singular name' ),
        ...
        'parent_item_colon' => '',
        'menu_name' => 'WTGCSVEXPORTER Content'
    );
    $args = array(
        'labels' => $labels,
        'public' => false,
...
function wtgcsvexporter_add_meta_boxes_post() {
    ...
    //        'wtgcsvexporter_metabox_authoradsense',
    //        'post',
    //        'side',
    //        'default'
    //    );
...
*/
function wtgcsvexporter_metabox_authoradsense( $object, $box ) { 
    //wp_nonce_field( basename( __FILE__ ), 'postnonce' );
    //$value = get_post_meta( $object->ID, '_c2ppost_authoradsense', true );
    //echo '<textarea id="c2pauthoradsense" name="wtgcsvexporter_post_authoradsense" rows="5" cols="30">'.$value.'</textarea>'; 
} 

/**
* Save meta box's for post posts type
*/
function wtgcsvexporter_save_meta_boxes_post( $post_id, $post ) {    
    ...
    {  
        ...
        if( isset( $_POST['wtgcsvexporter_post_'.$term] ) )
        {
            $new_meta_value = $_POST['wtgcsvexporter_post_'.$term];    
        }
