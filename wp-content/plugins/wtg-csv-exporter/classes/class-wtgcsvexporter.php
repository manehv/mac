 ...
 * and to class-wpcore.php, making the wtgcsvexporter easier to use for small to large projects.
...
* @author Ryan Bayne 
*/                                                 
class WTGCSVEXPORTER {
    
    /**
     * Page hooks (i.e. names) WordPress uses for the WTGCSVEXPORTER admin screens,
    ...
    */
    public function __construct() {
        ...
        $this->PHP = self::load_class( 'WTGCSVEXPORTER_PHP', 'class-phplibrary.php', 'classes' );
        $this->Install = self::load_class( 'WTGCSVEXPORTER_Install', 'class-install.php', 'classes' );
        $this->Files = self::load_class( 'WTGCSVEXPORTER_Files', 'class-files.php', 'classes' );
  
        $wtgcsvexporter_settings = self::adminsettings();
    ...
    */
    public function plugin_admin_register_styles() {
        wp_register_style( 'wtgcsvexporter_css_notification',plugins_url( 'wtgcsvexporter/css/notifications.css' ), array(), '1.0.0', 'screen' );
        wp_register_style( 'wtgcsvexporter_css_admin',plugins_url( 'wtgcsvexporter/css/admin.css' ), __FILE__);          
    }
    ...
    */
    public function plugin_admin_print_styles() {
        wp_enqueue_style( 'wtgcsvexporter_css_notification' );  
        wp_enqueue_style( 'wtgcsvexporter_css_admin' );               
    }    
    ...
    */
    ...
    public function enqueue_style( $name, array $dependencies = array() ) {
        ...
        $css_url = plugins_url( $css_file, WTGCSVEXPORTER__FILE__ );
        wp_enqueue_style( "wtgcsvexporter-{$name}", $css_url, $dependencies, WTGCSVEXPORTER::version );
    }
    ...
    public function enqueue_script( $name, array $dependencies = array(), $localize_script = false, $force_minified = false ) {
        ...
        $js_url = plugins_url( $js_file, WTGCSVEXPORTER__FILE__ );
        wp_enqueue_script( "wtgcsvexporter-{$name}", $js_url, $dependencies, WTGCSVEXPORTER::version, true );
    }  
    ...
    */
    public static function load_class( $class, $file, $folder, $params = null ) {
         ...
         */        
        $class = apply_filters( 'wtgcsvexporter_load_class_name', $class );
    ...
    /**
    * returns the WTGCSVEXPORTER_WPMain class object already created in this WTGCSVEXPORTER class
    * 
    * @author Ryan R. Bayne
    * @package WTG CSV Exporter
    * @since 0.0.1
    ...
    */
    ...
    public static function load_file( $file, $folder ) {   
        $full_path = WTGCSVEXPORTER_ABSPATH . $folder . '/' . $file;
         ...
         */
        $full_path = apply_filters( 'wtgcsvexporter_load_file_full_path', $full_path, $file, $folder );
        if ( $full_path ) {   
    ...
    public static function load_draggableboxes_view( $page_slug, array $data = array() ) {
        ...
        $the_view = self::load_class( "WTGCSVEXPORTER_{$ucview}_View", "{$page_slug}.php", $dir );
     ...
     */
     public function load_admin_page() {    
        ...
        $page = 'main';
        if( isset( $_GET['page'] ) && $_GET['page'] !== 'wtgcsvexporter' ){    
            $page = substr( $_GET['page'], strlen( 'wtgcsvexporter_' ) );
        }
    ...
    */
    private function filteraction_should_beloaded( $whenToLoad) {
        $wtgcsvexporter_settings = $this->adminsettings();
        ...
        switch( $whenToLoad) {
            ...
            case 'pluginscreens':
                ...
                // load when on a WTG CSV Exporter admin screen
                if( isset( $_GET['page'] ) && strstr( $_GET['page'], 'wtgcsvexporter' ) ){return true;}
            ...
            case 'systematicpostupdating':  
                if(!isset( $wtgcsvexporter_settings['standardsettings']['systematicpostupdating'] ) || $wtgcsvexporter_settings['standardsettings']['systematicpostupdating'] != 'enabled' ){
                    return false;    
                }      
                return true;
            break;
    ...
    */
    ...
    /**
    * Gets option value for wtgcsvexporter _adminset or defaults to the file version of the array if option returns invalid.
    * 1. Called in the main wtgcsvexporter.php file.
    ...
    */
    public function adminsettings() {
        $result = $this->option( 'wtgcsvexporter_settings', 'get' );
    ...
    * 
    * @uses WTGCSVEXPORTER_UI::add_dashboard_widgets() which has the widgets
    * 
    * @author Ryan R. Bayne
    * @package WTG CSV Exporter
    * @since 0.0.1
    ...
    */       
    ...
    public function screen_options() {
        ...
        // toplevel_page_wtgcsvexporter (main page)
        if( $screen->id == 'toplevel_page_wtgcsvexporter' ){
            $args = array(
                'label' => __( 'Members per page' ),
                'default' => 1,
                'option' => 'wtgcsvexporter_testoption'
            );
    ...
    public function save_screen_option( $status, $option, $value ) {
        if ( 'wtgcsvexporter_testoption' == $option ) return $value;
    }
    ...
    */
    public function help_tab () {
        ...
        $WTGCSVEXPORTER_Help = self::load_class( 'WTGCSVEXPORTER_Help', 'class-help.php', 'classes' );
        ...
        // if on main page "wtgcsvexporter" then set tab name as main
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'You are welcome to visit the', 'wtgcsvexporter' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewreadmoreurl' ] . '"';
            $help_content .= 'title="' . __( 'Visit the WTG CSV Exporter website and read more about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';
            $help_content .= __( 'WTG CSV Exporter Website', 'wtgcsvexporter' ) . '</a> ' . __( 'to read more about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] ) ){
            $help_content .= '<p>';
            $help_content .= __( 'There is a', 'wtgcsvexporter' ) . ' ';
            $help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewvideourl' ] . '"';
            $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= __( 'YouTube Video', 'wtgcsvexporter' ) . '</a> ' . __( 'about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ];           
            $help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewdiscussurl' ] ) ){
            ...
            $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '"';
            $help_content .= 'target="_blank"';
            $help_content .= '>';            
            $help_content .= $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'wtgcsvexporter' );           
            $help_content .= '.</p>';
        }         
        ...
        $screen->add_help_tab( array(
            'id'    => $page_name,
            'title'    => __( 'About', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'viewinfo' ][ 'viewtitle' ] ,
        ...
        foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ] as $form_id => $value ){
            ...
            if( isset( $value[ 'formreadmoreurl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Visit the WTG CSV Exporter website and read more about', 'wtgcsvexporter' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';
                $help_content .= __( 'WTG CSV Exporter Website', 'wtgcsvexporter' ) . '</a> ' . __( 'to read more about', 'wtgcsvexporter' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }  
            ...
            if( isset( $value[ 'formvideourl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'wtgcsvexporter' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= __( 'YouTube Video', 'wtgcsvexporter' ) . '</a> ' . __( 'about', 'wtgcsvexporter' ) . ' ' . $value[ 'formtitle' ];           
                $help_content .= '.</p>';
            }
            ...
            if( isset( $value[ 'formdiscussurl' ] ) ){
                ...
                $help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'wtgcsvexporter' ) . ' ' . $value[ 'formtitle' ] . '"';
                $help_content .= 'target="_blank"';
                $help_content .= '>';            
                $help_content .= $value[ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'wtgcsvexporter' );           
                $help_content .= '.</p>';
            } 
            ...
            foreach( $value[ 'options' ] as $key_two => $option_array ){  
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $help_content .= ' title="' . __( 'Read More about', 'wtgcsvexporter' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Read More', 'wtgcsvexporter' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $help_content .= ' title="' . __( 'Watch a video about', 'wtgcsvexporter' )  . ' ' . $option_array['optiontitle'] . '"';
                    $help_content .= ' target="_blank">';
                    $help_content .= __( 'Video', 'wtgcsvexporter' ) . '</a>';      
                }
            }
    ...
    * @version 1.0
    *  
    * @param mixed $wtgcsvexporter_page_name
    * @param mixed $default
    */
    ...
    public function build_tab_menu( $current_page_name ){  
        ...
        foreach( $wtgcsvexporter_menu_array as $page_name => $values ){
            ...
            if( $values['tabgroup'] === $view_group ){
                ...
                echo '<a href="' . self::create_adminurl( $values['longname'] ) . '" '.$activeclass.'>' . $values['pluginmenu'] . '</a>';       
            }
        }      
    ...
    * 
    * function was reduced to two lines, the contents mode to WTGCSVEXPORTER_Requests itself.
    *
    * @author Ryan R. Bayne
    * @package WTG CSV Exporter
    * @since 0.0.1
    ...
    */
    public function process_admin_POST_GET() {  
        ...
        $WTGCSVEXPORTER_REQ = self::load_class( 'WTGCSVEXPORTER_Requests', 'class-requests.php', 'classes' );
        $WTGCSVEXPORTER_REQ->process_admin_request();
    }  
    ...
    */
    public function pluginmediabutton_popup() {
        ...
        <div id="wtgcsvexporter_popup_container" style="display:none;">
    ...
    public function event_check() {
        ...
        {    
            ...
            if( $soonest > time() ){
                ...
                self::event_return( __( 'has not been 60 seconds since list event', 'wtgcsvexporter' ) ); 
                return;
            }             
        }
        else
        {               
            ...
            self::event_return( __( 'initialised the last event time value', 'wtgcsvexporter' ) );
            return;        
        }                             
        ...
        {    
            ...
            self::event_return( __( 'initialised last event type value', 'wtgcsvexporter' ) );
            return;
        }
        ...
        {    
            ...
            self::event_return( __( 'initialised last daily reset time', 'wtgcsvexporter' ) );        
            return;
        } 
        ...
        { 
            ...
            self::event_return( __( 'initialised hourly reset time', 'wtgcsvexporter' ) );
            return;
        }    
        ...
        if(!isset( $wtgcsvexporter_schedule_array['hours'][$hour] ) )
        {
            ...
            self::event_return( __( 'schedule not permitting hour', 'wtgcsvexporter' ) );        
            return;    
        }
        ...
        if( $wtgcsvexporter_schedule_array['history']['hourcounter'] >= $wtgcsvexporter_schedule_array['limits']['hour'] )
        {
            ...
            self::event_return( __( 'hours limit reached', 'wtgcsvexporter' ) );            
            return;        
        }
        ...
        if( $wtgcsvexporter_schedule_array['history']['daycounter'] >= $wtgcsvexporter_schedule_array['limits']['day'] )
        {
            ...
            self::event_return( __( 'days limit reached', 'wtgcsvexporter' ) );        
            return;       
        }
    ...
    * wtgcsvexporter solution. Passing value to this method, can be used to call more specific method
    * for an event.
    * 
    * @author Ryan R. Bayne
    * @package WTG CSV Exporter
    ...
    * Must call ths function within a function that checks that uses wtgcsvexporter_DOING_AJAX() first 
    * 
    * 1. If you add a new event type, you must also update wtgcsvexporter_tab1_pagecreation.php (Schedule), specifically Events Status panel
    * 2. Update event_action when adding a new event type
    * 3. Update the Event Types panel and add option for new event types
    * 4. Update wtgcsvexporter_form_save_eventtypes
    * 
    ...
    */
    public function event_decide() {
        ...
        {
            self::log_schedule(sprintf( __( 'The plugins ability to override the next due event type has been applied and then next event forced is %s.', 'wtgcsvexporter' ), $override_event), __( 'next event type override', 'wtgcsvexporter' ),1, 'scheduledeventcheck', __LINE__, __FILE__, __FUNCTION__);         
    ...
    public function event_action( $run_event_type){    
        ...
        switch ( $run_event_type) {
            ...
            case "postcreation":
                ...
                //wtgcsvexporter_event_return( 'post creation procedure complete' );
    ...
    */
    public function pluginmediabutton_button( $context) {
        //append the icon
        $context = "<a class='button thickbox' title='WTG CSV Exporter Column Replacement Tokens (CTRL + C then CTRL + V)'
        href='#TB_inline?width=400&inlineId=wtgcsvexporter_popup_container'>WTG CSV Exporter</a>";
        return $context;
    }  
    ...
    */
    public function install_admin_settings() {
        require_once( WTGCSVEXPORTER_ABSPATH . 'arrays/settings_array.php' );
    ...
    */
    public function newlog( $atts ){     
        ...
        if( isset( $wtgcsvexporter_settings['globalsettings']['uselog'] ) && $wtgcsvexporter_settings['globalsettings']['uselog'] == 0){
            return false;
        }
    ...
    */
    ...
    function conflict_prevention( $outputnoneactive = false ){
        ...
        $plugin_profiles[0]['title'] = __( 'Tweet My Post', 'wtgcsvexporter' );
        $plugin_profiles[0]['slug'] = 'tweet-my-post/tweet-my-post.php';
        $plugin_profiles[0]['author'] = 'ksg91';
        $plugin_profiles[0]['title_active'] = __( 'Tweet My Post Conflict', 'wtgcsvexporter' );
        $plugin_profiles[0]['message_active'] = __( 'Please deactivate Twitter plugins before performing mass post creation. This will avoid spamming Twitter and causing more processing while creating posts.', 'wtgcsvexporter' );
        $plugin_profiles[0]['message_inactive'] = __( 'If you activate this or any Twitter plugin please ensure the plugins options are not setup to perform mass tweets during post creation.', 'wtgcsvexporter' );
    ...
    */
    ...
    public function send_email( $recipients, $subject, $content, $content_type = 'html' ){     
        ...
        {
            add_filter( 'wp_mail_content_type', 'wtgcsvexporter_set_html_content_type' );
        }
        ...
        if( $content_type == 'html' )
        {    
            remove_filter( 'wp_mail_content_type', 'wtgcsvexporter_set_html_content_type' );  
        }   
    ...
    *  
    * @param mixed $page, registered page slug i.e. wtgcsvexporter_install which results in wp-admin/admin.php?page=wtgcsvexporter_install   
    ...
    */
    public function formend_standard( $buttontitle = 'Submit', $buttonid = 'notrequired' ){
            if( $buttonid == 'notrequired' ){
                $buttonid = 'wtgcsvexporter_notrequired'.rand(1000,1000000);# added during debug
            }else{
                $buttonid = $buttonid.'_formbutton';
            }?>

            <p class="submit">
                <input type="submit" name="wtgcsvexporter_wpsubmit" id="<?php echo $buttonid;?>" class="button button-primary" value="<?php echo $buttontitle;?>">
            </p>
    ...
    public function formstart_standard( $name, $id = 'none', $method = 'post', $class, $action = '', $enctype = '' ){
        ...
        echo '<form '.$class.' '.$enctype.' id="'.$id.'" method="'.$method.'" name="wtgcsvexporter_request_'.$name.'" action="'.$action.'">
        <input type="hidden" id="wtgcsvexporter_admin_action" name="wtgcsvexporter_admin_action" value="true">';
    } 
    ...
    */
    public function pageheader( $pagetitle, $layout ){
        ...
        <div id="wtgcsvexporter-page" class="wrap">
    ...
    */
    public function check_requirements( $display ){
        ...
        // php version
        if(defined(WTGCSVEXPORTER_PHPVERSIONMINIMUM) ){
            if(WTGCSVEXPORTER_PHPVERSIONMINIMUM > phpversion() ){
                ...
                if( $display == true){
                    ...
                    between PHP versions per WP installation or sometimes hosting allows customers to switch using their control panel.', 'wtgcsvexporter' ),WTGCSVEXPORTER_PHPVERSIONMINIMUM)
                    , 'warning', 'Large', __( 'WTG CSV Exporter Requires PHP ', 'wtgcsvexporter' ) . WTGCSVEXPORTER_PHPVERSIONMINIMUM);                
                }
            }
        }
    ...
    /**
    * Wrapper, uses wtgcsvexporter_url_toadmin to create local admin url
    * 
    * @param mixed $page
    * @param mixed $values 
    */
    ...
    */
    ...
    public function get_installed_version() {
        return get_option( 'wtgcsvexporter_installedversion' );    
    }  
    ...
    */
    public function update_option_schedule_array( $schedule_array ){
        ...
        return update_option( 'wtgcsvexporter_schedule', $schedule_array_serialized);    
    }
    
    public function update_settings( $wtgcsvexporter_settings ){
        $admin_settings_array_serialized = maybe_serialize( $wtgcsvexporter_settings );
        return update_option( 'wtgcsvexporter_settings', $admin_settings_array_serialized);    
    }
    ...
    */
    ...
    /**
    * Determines if the giving value is a WTG CSV Exporter page or not
    */
    public function is_plugin_page( $page){
        return strstr( $page, 'wtgcsvexporter' );  
    } 
    ...
    */
    ...
    public function create_localmedia_fromlocalimages( $file_url, $post_id ){           
        ...
        global $wpdb, $wtgcsvexporter_settings;
        ...
        if( isset( $wtgcsvexporter_settings['create_localmedia_fromlocalimages']['destinationdirectory'] ) ){   
            $artDir = $wtgcsvexporter_settings['create_localmedia_fromlocalimages']['destinationdirectory'];
        }else{
    ...
    */
    public function create_posts( $project_id, $total = 1 ){
        ...
        $autoblog = new WTGCSVEXPORTER_InsertPost();
        $autoblog->settings = $wtgcsvexporter_settings;
    ...
    */
    public function update_posts( $project_id, $total = 1, $post_id = false, $atts = array() ){
        ...
        $autoblog = new WTGCSVEXPORTER_UpdatePost();
        $autoblog->settings = $wtgcsvexporter_settings;
        $autoblog->maintable = $database_table;
    ...
    * @param mixed $level
    * 
    * @deprecated WTGCSVEXPORTER_Categories class created
    */
