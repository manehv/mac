...
class WTGCSVEXPORTER_UI extends WTGCSVEXPORTER {     
    ...
    public function __construct() {
        ...
        $this->WPCore = self::load_class( 'WTGCSVEXPORTER_WPCore', 'class-wpcore.php', 'classes' );  
    }  
          
    /**
    * Called by main WTGCSVEXPORTER() class using proper hook
    * 
    ...
    */
    public function add_dashboard_widgets() {
        ...
        wp_add_dashboard_widget(
             ...
             __( 'Test Widget', 'wtgcsvexporter' ),// Title.
        ...
        foreach( $wtgcsvexporter_menu_array as $key => $section_array ) {
            ...
            if( !isset( $wtgcsvexporter_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] ) ) {       
                continue;    
            } elseif( $wtgcsvexporter_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] !== 'enabled' ) {    
                continue;    
            }
            ...
            if( !isset( $wtgcsvexporter_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {      
                continue;
            } elseif( !current_user_can( $wtgcsvexporter_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {    
            ...
            $view_object = WTGCSVEXPORTER::load_class( 'WTGCSVEXPORTER_' . ucfirst( $section_array['name'] ) . '_View', $section_array['name'] . '.php', 'views' );
    ...
    */
    ...
    /**
    * Displays a table of wtgcsvexporter_option records with ability to view their value or delete them
    ...
    */
    public function list_optionrecordtrace( $form = false, $size = 'Small', $optiontype = 'all' ){ 
        // first get all records that begin with wtgcsvexporter_
        $wtgcsvexporterrecords_result = $this->DB->options_beginning_with( 'wtgcsvexporter_' );
        $counter = 1;?>
            
            <script language="JavaScript">
            function wtgcsvexporter_deleteoptions_checkboxtoggle(source) {
              checkboxes = document.getElementsByName( 'wtgcsvexporter_deleteoptions_array[]' );
        ...
        if( $form){
            echo '<input type="checkbox" onClick="wtgcsvexporter_deleteoptions_checkboxtoggle(this)" /> '. __( 'Select All Options', 'wtgcsvexporter' ) .'<br/>';
        }
        
        $html = '';
                        
        foreach( $wtgcsvexporterrecords_result as $key => $option ){
            
            if( $form){
                $html = '<input type="checkbox" name="wtgcsvexporter_deleteoptions_array[]" value="'.$option.'" />';
            }
        ...
        <div class="wtgcsvexporter_screenintro_container">
            <div class="welcome-panel">
                ...
                <div class="welcome-panel-content">
                    ...
                    <p>'.__( 'Pledge £9.99 to the WTG CSV Exporter project for 50% discount on the premium edition.' ).'</p>                                                     
                </div>
    ...
    */
    public function option_switch( $title, $name, $id, $current_value = 'nocurrent123', $onlabel = 'Enabled', $offlabel = 'Disabled', $default = 'enabled' ){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $title, 'wtgcsvexporter' ); ?></th>
            <td>
    ...
    * @deprecated use class-forms.php      
    */
    public function option_text( $title, $name, $id, $current_value = '', $readonly = false, $class = 'wtgcsvexporter_inputtext', $append_text = '', $left_field = false, $right_field_content = '', $required = false, $validation = false ){
    ...
    */
    public function option_text_simple( $title, $nameandid, $current_value = '', $required = false, $validation = false ){
        $this->option_text( $title, $nameandid, $nameandid, $current_value, false, 'wtgcsvexporter_inputtext', '', false, '', $required, $validation );
    }
    ...
    */
    public function metabox_icons( $form_id, $trash_icon = false ){
        ...
        if( isset( $help_array[ $page_name ][ WTGCSVEXPORTER_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] ) ){
            self::panel_video_icon( $help_array[ $page_name ][ WTGCSVEXPORTER_VIEWNAME ][ 'forms' ][ $form_id ][ 'formvideoid' ] );
            ++$total_icons;        
        } 

        // do we have all required values to build help content and display form_information_icon()
        if( isset( $help_array[ $page_name ][ WTGCSVEXPORTER_VIEWNAME ][ 'forms' ][ $form_id ][ 'formtitle' ] )
            && isset( $help_array[ $page_name ][ WTGCSVEXPORTER_VIEWNAME ][ 'forms' ][ $form_id ][ 'formabout' ] ) ){  
                           
            // display the icon that will show information about the box when clicked
            self::box_information_icon( $page_name, WTGCSVEXPORTER_VIEWNAME, $form_id );
        }
    ...
    public function panel_trash_icon( $form_name ){?>
        <a href="<?php echo $form_name;?>"><img src="<?php echo WTGCSVEXPORTER_IMAGES_URL . 'trash-icon.gif';?>" alt="<?php _e( 'Reset form' );?>" title="<?php _e( 'Reset form' );?>"></a><?php 
    }
    
    public function panel_video_icon( $youtubeid ){
        if( $youtubeid){
            echo '<a href="https://www.youtube.com/watch?v=' . $youtubeid . '" target="_blank"><img src="' . WTGCSVEXPORTER_IMAGES_URL . 'video-icon.gif" alt="' . __( 'View video on YouTube' ) . '" title="' . __( 'View video on YouTube' ) . '"></a>';
        }
    }
    ...
    */
    public function helpicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View help for this on the plugins website.' ) .'" target="_blank"><img src="'. WTGCSVEXPORTER_IMAGES_URL . 'help-icon.png" alt="'. __( 'View help for this on the plugins website' ) .'"></a>';
    }    
    public function videoicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View a video about this feature.' ) .'" target="_blank"><img src="'. WTGCSVEXPORTER_IMAGES_URL . 'video-icon.png" alt="'. __( 'Video icon, click to open a video' ) .'"></a>';
    }     
    public function trashicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Delete or hide the current feature.' ) .'" target="_blank"><img src="'. WTGCSVEXPORTER_IMAGES_URL . 'trash-icon.png" alt="'. __( 'Trash icon, click to delete or hide the current item' ) .'"></a>';
    }    
    public function discussicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Discuss this feature on the WebTechGlobal forum.' ) .'" target="_blank"><img src="'. WTGCSVEXPORTER_IMAGES_URL . 'chat-icon.png" alt="'. __( 'Chat icon which you can click to discuss this feature on the WebTechGlobal forum.' ) .'"></a>';
    }  
    ...
    */
    public function postbox_content_header( $form_title, $form_id, $introduction = false, $trash_icon = false, $uploader = false ) {?>
        <p>
            ...
            <?php if( $introduction !== false && !empty( $introduction) ){echo "<p class=\"wtgcsvexporter_boxes_introtext\">". $introduction ."</p>";}?>
    ...
    */
    public function postbox_content_footer( $submit_title = false ) {
        ...
        if( $submit_title === false ) {
            $submit_title = __( 'Submit', 'wtgcsvexporter' );    
        }
    ...
    */          
    public function box_information_icon( $page_name, $view_name, $form_id ) {
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'You are welcome to visit the', 'wtgcsvexporter' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the WTG CSV Exporter website and read more about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';
            $all_help_content .= __( 'WTG CSV Exporter Website', 'wtgcsvexporter' ) . '</a> ' . __( 'to read more about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'There is a', 'wtgcsvexporter' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] . '"';
            $all_help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= __( 'YouTube Video', 'wtgcsvexporter' ) . '</a> ' . __( 'about', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] ) ){
            ...
            $all_help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'wtgcsvexporter' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'wtgcsvexporter' );           
            $all_help_content .= '.</p>';
        }        
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] ) ){
            foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] as $option_id => $option_array ){
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $all_help_content .= ' title="' . __( 'Read More about', 'wtgcsvexporter' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Read More', 'wtgcsvexporter' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $all_help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $all_help_content .= ' title="' . __( 'Watch a video about', 'wtgcsvexporter' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Video', 'wtgcsvexporter' ) . '</a>';      
                }
        ...
        $all_help_content .= __( 'Help for ', 'wtgcsvexporter' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '">';
        $all_help_content .= '<img src="' . WTGCSVEXPORTER_IMAGES_URL . 'info-icon.gif" alt="';
        $all_help_content .= __( 'Click icon to view help for ', 'wtgcsvexporter' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"></a>';
    ...
    * 
    * @deprecated use wtgcsvexporter_n_postresult as of 13th February 2013
    */
    ...
    /**
    * New Notice Function (replacing WTGCSVEXPORTER_Notice::notice_depreciated() )
    ...
    */
    public function update_persistentnotifications_array( $notifications_array ){
        return update_option( 'wtgcsvexporter_notifications',maybe_serialize( $notifications_array ) );    
    }
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'www.webtechglobal.co.uk', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        ...
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $helpurl != false && $helpurl != '' && $helpurl != 'http://www.wtgcsvexporter.com/support' ){$clickable = false;}
    ...
    */
    ...
    public function add_admin_footer_text() {
        // show admin footer message (only on pages of WTG CSV Exporter)
        add_filter( 'admin_footer_text', array( $this, '_admin_footer_text' ) );
    }
    
    /**
     * Add a WTG CSV Exporter "Thank You" message to the admin footer content
     *
     * @since 0.0.1
     *
    ...
    */
    public function register_input_validation( $input_title, $input_name, $input_id, $input_validation ) {
        ...
        $form_val_arr = get_option( 'wtgcsvexporter_formvalidation' );
        ...
        update_option( 'wtgcsvexporter_formvalidation', $form_val_arr );
    }
    ...
    */
    public function metabox_nocurrentproject() {
        ...
        the one that was active.', 'wtgcsvexporter' );
        echo '</p>';
    }
