<?php
/**
 * Members and methods for WTG CSV Exporter views that use post boxes (eventually all views)
 *
...
/**
 * WTG CSV Exporter Base View class
 * 
 * Taking from WTGCSVEXPORTER plugin and customized to pave the way for postboxes on multiple
 ...
 * @version 1.0.2
 */
abstract class WTGCSVEXPORTER_View {
        ...
        if( isset( $this->help_array[ $page_name ] ) ){
            ...
            if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pageabout' ] ) ){
                ...
                $screen->add_help_tab( array(
                    'id' => $page_name,
                    'title' => __( 'About', 'wtgcsvexporter' ) . ' ' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagetitle' ],
                ...
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] ) ){
                    $pagereadmeurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagereadmoreurl' ] . '" target="_blank">' . __( 'Documentation', 'wtgcsvexporter') . '</a></p>';        
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] ) ){
                    $pagevideourl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagevideourl' ] . '" target="_blank">' . __( 'Video', 'wtgcsvexporter') . '</a></p>';
                } 
                               
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] ) ){
                    $pagediscussionurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagediscussurl' ] . '" target="_blank">' . __( 'Support', 'wtgcsvexporter') . '</a></p>';
                }
        
                if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ] ) ){
                    $pagefaqurl = '<p><a href="' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagefaqurl' ]  . '" target="_blank">' . __( 'FAQ', 'wtgcsvexporter') . '</a></p>';
                }
                                
                // help tab sidebar
                $screen->set_help_sidebar( '
                <p><strong>' . __( 'For more information:', 'wtgcsvexporter' ) . '</strong></p>' . $pagereadmeurl . $pagevideourl . $pagediscussionurl . $pagefaqurl );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        $this->WTGCSVEXPORTER->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
        ...
        $long_id = "wtgcsvexporter_{$this->action}-{$id}";
    ...
    */
    protected function add_meta_box( $id, $title, $callback, $context = 'normal', $priority = 'default', $callback_args = null ) {
        ...
        $id = "wtgcsvexporter_{$this->action}-{$id}";         
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->WTGCSVEXPORTER->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    protected function action_field( array $data, array $box ) {
        echo "<input type=\"hidden\" name=\"action\" value=\"wtgcsvexporter_{$this->action}\" />\n";
    }
    ...
    */
    protected function textbox_submit_button( array $data, array $box ) {
        $caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'wtgcsvexporter' );
        ?>
