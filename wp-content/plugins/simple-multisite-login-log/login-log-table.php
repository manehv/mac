...
class LoginLogTable extends WP_List_Table {
	...
	$query = "SELECT * FROM $this->mTable WHERE blog_id='$blog_id'";
	...
	switch ( $column_name ) {
		...
		if ( is_numeric( $item['uid'] ) && $item['uid'] > 0 ) {
		    ...
		    return $user->display_name . $this->row_actions( array( '<a href="'.admin_url().'user-edit.php?user_id='.$user->ID.'&wp_http_referer=%2Fwp-admin%2Fusers.php?page=snll_admin_menu">Edit</a>', '<a href="'.wp_nonce_url( admin_url().'users.php?action=remove&user='.$user->ID, 'bulk-users' ).'&wp_http_referer=%2Fwp-admin%2Fusers.php?page=snll_admin_menu">Remove</a>' ) );
		}
		else
		    return '';
		break;
