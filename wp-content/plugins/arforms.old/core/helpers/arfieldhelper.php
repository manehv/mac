...
class arfieldhelper{
    ...
    function _show_category($atts) {
    	...
    	<div class="frm_<?php echo $type ?>" id="frm_<?php echo $type .'_'. $sanitized_name ?>">
    	    ...
    	    <label<?php echo $class ?> for="field_<?php echo $sanitized_name ?>"><input type="<?php echo $type ?>" name="<?php echo $field_name ?>" <?php echo (isset($hide_id) and $hide_id)? '' : 'id="field_'. $sanitized_name .'"'; ?> value="<?php echo $cat->cat_ID ?>" <?php echo $checked;  do_action('arffieldinputhtml', $field); ?> /><?php echo $cat->cat_name ?></label>
...
<?php       }
    ...
    function get_linked_options($values, $field, $entry_id=false){
        ...
        foreach ($metas as $meta){
            ...
            if(empty($meta['entry_value'])) continue;
...
[if description]<div class="arf_field_description arf_heading_description" [description_style]>[description]</div>[/if description]
    ...
    function replace_field_shortcodes($html, $field, $errors=array(), $form=false){
        ...
        foreach ($shortcodes[0] as $short_key => $tag){
            ...
            }else if($tag == 'deletelink' and class_exists('arrecordcontroller'))


                $replace_with = $arrecordcontroller->entry_delete_link($atts);
...
<?php       echo '<div id="recaptcha_style">'.recaptcha_get_html($arfsettings->pubkey .'&hl='. $lang, $error, is_ssl()).$arfieldhelper->replace_description_shortcode($field).'</div>';
    ...
    function before_replace_shortcodes($html, $field,$error,$form){
        ...
        if(isset($field['align']) and ($field['type'] == 'radio' or $field['type'] == 'checkbox')){
            ...
            if(($field['type'] == 'radio' and $field['align'] != $arfradioalignsetting) or 


                ($field['type'] == 'checkbox' and $field['align'] != $arfcheckboxalignsetting)){
    ...
    function get_shortcode_select($form_id, $target_id='content', $type='all', $style=''){ 
       ...
       ?>
        ...
        <select class="frm_shortcode_select" onchange="arfaddcodefornewfield('<?php echo $target_id ?>',this.value);this.value='';" <?php echo (isset($style))? $style : '';?> data-width='330px'>
    ...
    function replace_shortcodes($content, $entry, $shortcodes, $display=false, $show='one', $odd=''){
        ...
        if($display){
            ...
            $param_value = ($display->type == 'id') ? $entry->id : $entry->entry_key;
            ...
            if($entry->attachment_id){
                ...
                $detail_link = get_permalink($entry->attachment_id);
            ...
            switch($tag){                
                ...
                default:
                    ...
                    if($tag == 'deletelink'){
                        ...
                        $can_delete = $arrecordhelper->allow_delete($entry);
                        ...
                        if($can_delete){
                            ...
                            if(isset($atts['label'])){
                                ...
                                $replace_with = $arrecordcontroller->entry_delete_link($delete_atts);
                                ...
                                $replace_with = add_query_arg(array('arfaction' => 'destroy', 'entry' => $entry->id), get_permalink($page_id));
	...
	function get_actual_id($field_id){
		...
		$id = ( $res->ref_field_id > 0 ) ? $res->ref_field_id : $field_id;
		return $id;
	}
	...
	function get_field_defautl_value( $field )
	{
		...
		if((isset($field_options['clear_on_focus']) and $field_options['clear_on_focus'] and !empty($field['default_value']))){ 
			...
			if($field_options['default_blank']==1){ $value1 = trim($armainhelper->esc_textarea($field['default_value'])); }
	...
	function get_shortcode_modal($form_id, $target_id='content', $type='all', $style='', $is_total_field =false){ 
       ...
       ?>
			...
			if($type == 'email' && $target_id != 'options_admin_reply_to_notification' && $target_id != 'ar_admin_from_email' && $target_id != 'ar_user_from_email' )
			{ 
			...
			}else
			{
				if( $is_total_field ){
					if($field->type != 'html')
					{
	        			?><div class="modal_field_val" id="arfmodalfieldval_<?php echo $field->ref_field_id;?>" onclick="arfaddtotalfield('<?php echo $target_id;?>','<?php echo $field->ref_field_id;?>','')"><?php echo $field_name = $armainhelper->truncate($field->name, 60).'</div>'; 
					}
				}
				else
				{
					if($target_id=="ar_email_subject" && $field->type != 'html')
					{
						?><div class="modal_field_val" id="arfmodalfieldval_<?php echo $field->ref_field_id;?>" onclick="arfaddcodefornewfield('<?php echo $target_id;?>','<?php echo $field->ref_field_id;?>')"><?php echo $field_name = $armainhelper->truncate($field->name, 60).'</div>'; 
			...
			if(!empty($field_list)){
				...
				foreach ($field_list as $field){ 
					...
					if($field->type == "checkbox")
					{
						...
						?>
                        <div class="modal_field_val" id="arfmodalfieldval_<?php echo $field->ref_field_id;?>" onclick="javascript:return false;"><strong><?php echo $field_name = $armainhelper->truncate($field->name, 40).'</div></strong>';
	...
	function arf_get_field_option_value($field,$option)
	{
			...
			?>
				<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
						...
						<tr class="fieldoptions_field_style">
                            ...
                            <input type="hidden" name="field_options[frm_clear_field_indicator_<?php echo $field['id'] ?>]" value="<?php $field['frm_clear_field_indicator'] = isset($field['frm_clear_field_indicator']) ? $field['frm_clear_field_indicator'] : ''; echo esc_attr($field['frm_clear_field_indicator']); ?>" />
                            </td>
						</tr>
				   </table>
			...
			?>
				<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                        ...
                        <input type="hidden" name="field_options[frm_default_blank_indicator_<?php echo $field['id'] ?>]" value="<?php $field['frm_default_blank_indicator'] = isset($field['frm_default_blank_indicator']) ? $field['frm_default_blank_indicator'] : ''; echo esc_attr($field['frm_default_blank_indicator']); ?>" />
                        </td>
                    </tr>
               </table>	
				...
				<table style="float:left;" border="0" cellpadding="0" cellspacing="0">  	
                     ...
                     <tr class="fieldoptions_field_style">
                     	<td class="sltstandard" style="float:none;">
                                ...
                                <input id="arf_field_align_<?php echo $field['id'];?>" name="field_options[align_<?php echo $field['id'] ?>]" value="<?php if($field['align']=='inline') { echo 'inline'; }else { echo 'block'; }?>" type="hidden" onchange="arfchangefieldalign('<?php echo $field['id'] ?>');">
                                <dl class="arf_selectbox" data-name="field_options[align_<?php echo $field['id'] ?>]" data-id="arf_field_align_<?php echo $field['id'];?>" style="width:160px;">
                                  <dt><span><?php if($field['align']=='inline') { echo __('Single Row', 'ARForms'); }else { echo __('Multiple Rows', 'ARForms'); }?></span>
                                    <input value="<?php if($field['align']=='inline') { echo __('Single Row', 'ARForms'); }else { echo __('Multiple Rows', 'ARForms'); }?>" style="display:none;width:148px;" class="arf_autocomplete" type="text">
                                    <i class="fa fa-caret-down fa-lg"></i></dt>
                                  <dd>
		...
		<?php /*?><select name="field_options[is_recaptcha_<?php echo $field['id'];?>]" id="is_recaptcha-<?php echo $field['id'];?>" class="frm-bulk-select-class" onchange="changerecaptchaimage('<?php echo $field['id'];?>',this.value, '<?php echo $field['id'];?>')" data-width="150px">
			...
			<option value="custom-captcha" <?php selected($is_recaptcha, 'custom-captcha') ?>>Default captcha</option>
                            ...
                            <input class="frm-bulk-select-class" id="is_recaptcha-<?php echo $field['id'];?>" name="field_options[is_recaptcha_<?php echo $field['id'];?>]" value="<?php if($is_recaptcha=='custom-captcha') { echo 'custom-captcha'; }else { echo 'recaptcha'; }?>" type="hidden" onchange="changerecaptchaimage('<?php echo $field['id'];?>',this.value, '<?php echo $field['id'];?>')">
                            <dl class="arf_selectbox" data-name="field_options[is_recaptcha_<?php echo $field['id'];?>]" data-id="is_recaptcha-<?php echo $field['id'];?>" style="width:140px;">
                              <dt><span><?php if($is_recaptcha=='custom-captcha') { echo 'Default captcha'; }else { echo 'Recaptcha'; }?></span>
                                <input value="<?php if($is_recaptcha=='custom-captcha') { echo 'Default captcha'; }else { echo 'Recaptcha'; }?>" style="display:none;width:128px;" class="arf_autocomplete" type="text">
                                <i class="fa fa-caret-down fa-lg"></i></dt>
                              <dd>
        ...
        <div style="font-size:14px; padding:5px 5px 5px 15px;">[ <?php _e('Embedded tags for youtube, map etc are supported.', 'ARForms');?> ]</div>
        			<div style="font-size:14px; padding:5px 5px 5px 15px;"><input type="checkbox" class="chkstanard" id="arfenable_total_<?php echo $field['id'];?>" name="field_options[enable_total_<?php echo $field['id'] ?>]" value="1" <?php checked($field['enable_total'], 1); ?> onchange="arf_show_runnig_total('<?php echo $field['id'];?>');" /><label for="arfenable_total_<?php echo $field['id'];?>"><span></span><?php _e('Enable Running Total', 'ARForms'); ?></label>                    
                    	<div class="arf_field_list_total_<?php echo $field['id'];?> arf_runnigtotal_block" style=" <?php if($field['enable_total'] != 1){ echo 'display:none;'; }?>">
	   ...
	   <?php } ?>
                    ...
                    <tr class="fieldoptions_field_style">
                        <td style="float:left">
                           ...
                           <?php /*?><select class="page_break_select" onchange="change_page_break_select(this.value);" name="field_options[page_break_type_<?php echo $field['id'] ?>]" style="width:180px;" data-width="180px">
                            ...
                            <input id="field_page_break_type_<?php echo $field['id'] ?>" class="page_break_select" name="field_options[page_break_type_<?php echo $field['id'] ?>]" value="<?php if($default_selected_pg_brk_type=='survey') { echo 'survey'; }else { echo 'wizard'; }?>" type="hidden" onchange="change_page_break_select(this.value);">
                            	...
                            	<dl class="arf_selectbox" data-name="field_options[page_break_type_<?php echo $field['id'] ?>]" data-id="field_page_break_type_<?php echo $field['id'] ?>" style="width:160px;">
                              		...
                              		<dt class="field_page_break_type_<?php echo $field['id'] ?>_dt"><span><?php if($default_selected_pg_brk_type=='survey') { echo __('Survey', 'ARForms'); }else { echo __('Wizard', 'ARForms'); }?></span>
                                		<input value="<?php if($default_selected_pg_brk_type=='survey') { echo __('Survey', 'ARForms'); }else { echo __('Wizard', 'ARForms'); }?>" style="display:none;width:148px;" class="arf_autocomplete" type="text">
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[confirm_password_label_<?php echo $field['id'];?>]" id="confirm_password_label_<?php echo $field['id'];?>" value="<?php echo $field['confirm_password_label']; ?>" style="width:210px;" <?php if( !isset($field['confirm_password']) or !$field['confirm_password'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
             	</table>
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[confirm_email_label_<?php echo $field['id'];?>]" id="confirm_email_label_<?php echo $field['id'];?>" value="<?php echo $field['confirm_email_label']; ?>" style="width:210px;" <?php if( !isset($field['confirm_email']) or !$field['confirm_email'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
             	</table>
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[invalid_confirm_email_<?php echo $field['id'];?>]" id="invalid_confirm_email_<?php echo $field['id'];?>" value="<?php echo $field['invalid_confirm_email']; ?>" style="width:210px;" <?php if( !isset($field['confirm_email']) or !$field['confirm_email'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
                </table>
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[confirm_email_placeholder_<?php echo $field['id'];?>]" id="confirm_email_placeholder_<?php echo $field['id'];?>" value="<?php echo $field['confirm_email_placeholder']; ?>" style="width:185px;" <?php if( !isset($field['confirm_email']) or !$field['confirm_email'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
             	</table>
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[invalid_password_<?php echo $field['id'];?>]" id="invalid_password_<?php echo $field['id'];?>" value="<?php echo $field['invalid_password']; ?>" style="width:210px;" <?php if( !isset($field['confirm_password']) or !$field['confirm_password'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
                </table>
				...
				<table style="float:left;">
                    ...
                    <tr class="fieldoptions_field_style">
                    	<td>
                            ...
                            <input type="hidden" name="field_options[frm_show_year_month_calendar_field_indicator_<?php echo $field['id'] ?>]" value="<?php $field['show_year_month_calendar'] = isset($field['show_year_month_calendar']) ? $field['show_year_month_calendar'] : ''; echo esc_attr($field['show_year_month_calendar']); ?>" />
                        </td>
                    </tr>
               </table>
			...
			?>
            	<table style="float:left;" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr class="fieldoptions_field_style">
                         <td style="float:left"><div style="float:left;"><input type="text" class="txtstandardnew" name="field_options[password_placeholder_<?php echo $field['id'];?>]" id="password_placeholder_<?php echo $field['id'];?>" value="<?php echo $field['password_placeholder']; ?>" style="width:185px;" <?php if( !isset($field['confirm_password']) or !$field['confirm_password'] ) { echo 'disabled="disabled"'; }?> /></div>
                         </td>
                    </tr>
             	</table>
