...
class arrecordmodel{
    ...
    function create( $values ){
		...
		$form_conditional_logic_arr = array();
		if(count($fields)>0)
		{
			foreach($fields as $conditional_val)
			{
				$field_conditional_logic = maybe_unserialize($conditional_val->conditional_logic);
				//print_r($field_conditional_logic);
				
				$form_conditional_logic_arr[$conditional_val->id] = $field_conditional_logic;
				
				$form_conditional_logic_arr[$conditional_val->id]["id"] = $conditional_val->id;
				$form_conditional_logic_arr[$conditional_val->id]["type"] = $conditional_val->type;
				$form_conditional_logic_arr[$conditional_val->id]["required"] = $conditional_val->required;
				$form_conditional_logic_arr[$conditional_val->id]["blank"] = $conditional_val->field_options["blank"];
			}
			//echo "<pre>";print_r($form_conditional_logic_arr);
			//exit;
			
			
			/*
			foreach($form_conditional_logic_arr as $conditioal_logic_field)
			{
				if($conditioal_logic_field["required"]>0)
				{	//echo "<pre>";print_r($conditioal_logic_field);
					//exit;
					if($conditioal_logic_field["enable"]<1)
					{	
						
						//echo "data=>";print_r($conditioal_logic_field);echo "|";print_r($posted_item_fields[$conditioal_logic_field["id"]]);
						//exit;
						if(is_array($posted_item_fields[$conditioal_logic_field["id"]]))
						{
							if(count($posted_item_fields[$conditioal_logic_field["id"]])<1)
							{	
								return false;
								break;
							}
						}
						else if($posted_item_fields[$conditioal_logic_field["id"]]=="")
						{	
					...
					else if($conditioal_logic_field["enable"]>0) 
					{
					
						
						$conditioal_logic_field_rules = $conditioal_logic_field["rules"];
						if($conditioal_logic_field["display"]=="show" && $conditioal_logic_field["if_cond"]=="all")
						//if($conditioal_logic_field["if_cond"]=="all")
						{
							foreach($conditioal_logic_field_rules as $conditioal_logic_field_rule)
							{
								if($conditioal_logic_field_rule["operator"]=="is")
								{
									if(($conditioal_logic_field_rule["value"]==$posted_item_fields[$conditioal_logic_field_rule["field_id"]]))
									{	
										//echo $conditioal_logic_field_rule["value"]."==". $posted_item_fields[$conditioal_logic_field_rule["field_id"]];
										
										return false;
										break;
									}								}
								else if($conditioal_logic_field_rule["operator"]=="is not")
								{//echo "<pre>";print_r($conditioal_logic_field);
								//echo '<br>if(('.$conditioal_logic_field_rule["value"].'!='.$posted_item_fields[$conditioal_logic_field_rule["field_id"]].'))';
							//exit;
								...
								else if($conditioal_logic_field_rule["operator"]=="greater than")
								{
									if( ( $posted_item_fields[$conditioal_logic_field_rule["field_id"]]>$conditioal_logic_field_rule["value"] ) )
									//if( ( $conditioal_logic_field_rule["value"]>$posted_item_fields[$conditioal_logic_field_rule["field_id"]] ) )
									{
								...
								else if($conditioal_logic_field_rule["operator"]=="less than")
								{
									if( ( $posted_item_fields[$conditioal_logic_field_rule["field_id"]] < $conditioal_logic_field_rule["value"]  ) )
									//if( !( $conditioal_logic_field_rule["value"] < $posted_item_fields[$conditioal_logic_field_rule["field_id"]] ) )
									{
								...
								else if($conditioal_logic_field_rule["operator"]=="contains")
								{
									if((@strpos($posted_item_fields[$conditioal_logic_field_rule["field_id"]],$conditioal_logic_field_rule["value"]) !== false))
									{	
										return false;
										break;
									}
								}
								else if($conditioal_logic_field_rule["operator"]=="not contains")
								{
									if(!(@strpos($posted_item_fields[$conditioal_logic_field_rule["field_id"]],$conditioal_logic_field_rule["value"]) !== false))
									{	
						...
						else if($conditioal_logic_field["display"]=="show" && $conditioal_logic_field["if_cond"]=="any")
						//if($conditioal_logic_field["if_cond"]=="all")
						{
							
							
							$chk_conditional_any_field = "";
							foreach($conditioal_logic_field_rules as $conditioal_logic_field_rule)
							{
								if($conditioal_logic_field_rule["operator"]=="is")
								{
							...
							if($chk_conditional_any_field=="1")
							{
								return false;
							}
						
						}
						else if($conditioal_logic_field["display"]=="hide" && $conditioal_logic_field["if_cond"]=="all") {
						...
						else if($conditioal_logic_field["display"]=="hide" && $conditioal_logic_field["if_cond"]=="any") {
