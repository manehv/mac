...
<?php /*?>function DoShow()
{
	...
	jQuery("#iframediv").html(' ').append('<iframe style="height:100%; display:block; width:100%; margin-top:0px;" frameborder="0" name="test" id="testiframe" src="<?php echo $pre_link."&title=true&description=true";?>" hspace="0"></iframe>');
...
function DoShow1(checkstatus)
{
	...
	var newwidth =  screenWidth - (385+157)<?php if($GLOBALS['wp_version'] < 3.1){ echo '-(14)'; }?>;
	...
	arfsack.setVar( "form", jsondata );
	arfsack.onError = function() { alert('<?php echo esc_js(__("Ajax error while saving form", "ARForms")) ?>' )};
	...
	/*nvals['form_id'] = <?php echo $id; ?>;
	nvals['action'] = 'arfformsavealloptions';
	nvals = JSON.stringify( nvals );
	nvals = nvals.replace(/&/g,'[AND]');
	nvals = nvals.replace(/\+/g,'[PLUS]');
	jQuery.ajax({type:"POST",url:ajaxurl,data:"action=arfformsavealloptions&form_id=<?php echo $id;?>&nforms="+nvals,
...
<?php /*?>function DoShow2(checkstatus, selectedval)
{
	jQuery("#iframediv").html(' ').append('<iframe style="height:100%; display:block; width:100%; margin-top:0px;" frameborder="0" name="test" id="testiframe" src="<?php echo $pre_link."&title=true&description=true&is_editorform=1&checkbox_radio_style_requested=";?>'+selectedval+'" hspace="0"></iframe>');
...
function arfmainformedit( is_addtosite_page ){
		...
		else 
		{
			if(sucmessage!="")
			{
				if( is_addtosite_page == 1 ) {							
					...
					setTimeout(function(){ jQuery('#success_message').remove(); jQuery('#arfsubmitall').attr('disabled', false); jQuery('#arfaddtosubmit').attr('disabled', false); },4000);						
				} else {							
					jQuery('#form_name_message').css("display","none");
	...
	nvals['form_preview'] = form_preview;
	nvals['action'] = 'arfformsavealloptions';
	nvals = JSON.stringify( nvals );
	nvals = nvals.replace(/&/g,'[AND]');
	nvals = nvals.replace(/\+/g,'[PLUS]');
	...
	jQuery.ajax({type:"POST",url:ajaxurl,evalScripts:true,data:"action=arfformsavealloptions&form_id="+form_id+"&form_preview="+form_preview+"&nforms="+nvals,
			success:function(msg){
        ...
        <div style=" <?php echo $controller_style; ?>" class=" <?php echo $field_editor_controller_style; ?>">
                    ...
                    <div id="arf_autoresponse" class="arfsettingsubcontainer">
                        ...
                        <div class="arfformtable">
                                ...
                                <div class="arfcolmnleft" style=' <?php echo $txt_align; ?>'>
                                    ...
                                    <div class="arftablerow">
                                        ...
                                        <div class="arfcolumnright" style=" <?php echo $tab_row_style; ?>">
                                            ...
                                            <?php /*?><select name="options[ar_email_to]" id="options_ar_user_email_to" class="arf_email_field_dropdown" style="width:170px;" data-width="170px" data-size="10" <?php echo $auto_responder_disabled;?>>
                                                <option value=""><?php _e('Select Field','ARForms');?></option>
                                                <?php 
                                                if(!empty($values['fields'])){
                                                foreach($values['fields'] as $val_key => $fo){
                                                    if(in_array($fo['type'], array('email','text', 'hidden'))){ ?>
                                                        <option value="<?php echo ($fo["ref_field_id"] > 0 ) ? $fo["ref_field_id"] : $fo["id"];?>" <?php if( ($fo["id"]==$values['ar_email_to']) || ($fo["ref_field_id"]==$values['ar_email_to']) ){?> selected="selected" <?php }?>><?php echo $fo['name'] ?></option>
                                                <?php }
                                                	}
                                                } ?>
                                            </select><?php */?>
									...
									?>
												...
												<input id="options_ar_user_email_to" name="options[ar_email_to]" value="<?php if($responder_email!="") { echo $responder_email; }else { echo $user_responder_email; } ?>" type="hidden" <?php echo $auto_responder_disabled;?>>
												<dl class="arf_selectbox" data-name="options[ar_email_to]" data-id="options_ar_user_email_to" style="width:160px;">
												  <dt class="options_ar_user_email_to_dt <?php if($auto_responder_disabled!="") { echo 'arf_disable_selectbox'; } ?>"><span><?php if( $selectbox_field_value_label!="" ) { echo $selectbox_field_value_label; } else { echo __('Select Field', 'ARForms'); }?></span>
													<input value="<?php if($responder_email!="") { echo $responder_email; }else { echo $user_responder_email; } ?>" style="display:none;width:148px;" class="arf_autocomplete" type="text">
                                ...
                                <div class="arfcolmnleft" style=' <?php echo $txt_align; ?>'>
                                    ...
                                    <div class="arftablerow">
                                        ...
                                        <div class="arfcolumnright">
                                                ...
                                                <input type="text" name="options[admin_email_subject]" id="admin_email_subject" size="55" class="txtstandardnew" value="<?php echo (isset($values['admin_email_subject'])) ? esc_attr($values['admin_email_subject']) : ''; ?>" style="width:297px;" <?php echo $chk_admin_notification_disabled;?>/>
                                                <div style="margin-top: 5px;">
                            ...
                            <div style="display:none;">
                            	...
                            	<input type="checkbox" class="chkstanard" name="options[inc_user_info]" id="inc_user_info" value="1" <?php $values['inc_user_info'] = isset($values['inc_user_info']) ? $values['inc_user_info'] : ''; checked($values['inc_user_info'], 1); ?> />
                            	<input type="checkbox" class="chkstanard" name="options[plain_text]" id="plain_text" value="1" <?php $values['plain_text'] = isset($values['plain_text']) ? $values['plain_text'] : ''; checked($values['plain_text'], 1); ?> />
                            </div>
                        ...
                        <div class="arfformtable">
                            ...
                            <div class="arfemailmarketdiv">
                        		<div class="arfemailmarketbar">
                                	<div id="arfem_mailchimp" class="arfemailbars <?php if($current_active_ar == 'mailchimp') { echo "arfactive"; }?>" onclick="arfshowem('mailchimp');"><div class="arfmailchimpimg"></div></div>
                                    <div id="arfem_aweber" class="arfemailbars <?php if($current_active_ar == 'aweber') { echo "arfactive"; }?>" onclick="arfshowem('aweber');"><div class="arfaweberimg"></div></div>
                                    <div id="arfem_icontact" class="arfemailbars <?php if($current_active_ar == 'icontact') { echo "arfactive"; }?>" onclick="arfshowem('icontact');"><div class="arficontactpimg"></div></div>
                                    <div id="arfem_constant_contact" class="arfemailbars <?php if($current_active_ar == 'constant_contact') { echo "arfactive"; }?>" onclick="arfshowem('constant_contact');"><div class="arfconstantimg"></div></div>
                                    <div id="arfem_getresponse" class="arfemailbars <?php if($current_active_ar == 'getresponse') { echo "arfactive"; }?>" onclick="arfshowem('getresponse');"><div class="arfgetresponseimg"></div></div>
                                    <div id="arfem_ebizac" class="arfemailbars <?php if($current_active_ar == 'ebizac') { echo "arfactive"; }?>" onclick="arfshowem('ebizac');"><div class="arfebizacimg"></div></div>
                                    <div id="arfem_gvo" class="arfemailbars <?php if($current_active_ar == 'gvo') { echo "arfactive"; }?>" onclick="arfshowem('gvo');"><div class="arfgvoimg"></div></div>
                                </div>
                                
                                <div style="clear:both; height:10px;"></div>
                                <div style="clear:both;">
                                	
                                    <div id="arfem_mailchimp_div" class="arfemdiv" <?php if($current_active_ar == 'mailchimp') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['mailchimp_type'] != 2 ) { ?>
        										<input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_1" value="1" checked="checked" onchange="show_setting('mailchimp', '1');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?> /><label for="autores_1"><span class="ar_lbl_span"></span><?php _e('Mailchimp', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Mailchip with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            	<input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_1" value="1" <?php if( isset($mailchimp_arr['enable']) && $mailchimp_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('mailchimp', '1');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?> /><label for="autores_1"><span class="ar_lbl_span"></span><?php _e('Mailchimp', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Mailchip with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                    ...
                                    <div id="arfem_aweber_div" class="arfemdiv" <?php if($current_active_ar == 'aweber') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['aweber_type'] != 2 ) { ?>
        		 							<input type="checkbox" class="chkstanard" style="width: 227px;height: 30px;margin-left: -5px;margin-top: -4px;" name="autoresponders[]" id="autores_3" value="3" checked="checked" onchange="show_setting('aweber', '3');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_3"><span class="ar_lbl_span"></span><?php _e('Aweber', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Aweber with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            	<input type="checkbox" class="chkstanard" style="width: 227px;height: 30px;margin-left: -5px;margin-top: -4px;" name="autoresponders[]" id="autores_3" value="3" <?php if( isset($aweber_arr['enable']) && $aweber_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('aweber', '3');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_3"><span class="ar_lbl_span"></span><?php _e('Aweber', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Aweber with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
												...
												//echo '<select name="i_aweber_list" id="i_aweber_list" style="width:180px;" data-width="180px" '.( $setact != 1 ? "readonly=readonly" : '' ).'>';
														...
														if( count($aweber_lists_name)>0 && is_array($aweber_lists_name) )
														{
															foreach($aweber_lists_name as $aweber_lists_name1) {
																...
																if($aweber_lists_id[$i]!="") {
																	/*?>
																	<option value="<?php echo $aweber_lists_id[$i];?>" <?php if($aweber_lists_id[$i]==$aweber_arr['type_val']){ echo "selected=selected"; }?>><?php echo $aweber_lists_name1;?></option>
																	 <?php  */
                                    ...
                                    <div id="arfem_icontact_div" class="arfemdiv" <?php if($current_active_ar == 'icontact') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['icontact_type'] != 2 ) { 
											$autochecked = '';
											if( $res['icontact_type'] == 1 and $res6[0]['is_verify']== 1 )
												...
												$autochecked = 'checked="checked"';		   
											?>
             								<input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_8" value="8" <?php echo $autochecked; ?> onchange="show_setting('icontact', '8');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_8"><span class="ar_lbl_span"></span><?php _e('Icontact', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Icontact with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_8" value="8" <?php if( isset($icontact_arr['enable']) && $icontact_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('icontact', '8');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_8"><span class="ar_lbl_span"></span><?php _e('Icontact', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Icontact with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                    ...
                                    <div id="arfem_constant_contact_div" class="arfemdiv" <?php if($current_active_ar == 'constant_contact') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['constant_type'] != 2 ) { 
											$autochecked = '';
											...
											?>    
            								<input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_9" value="9" <?php echo $autochecked;?> onchange="show_setting('constant', '9');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_9"><span class="ar_lbl_span"></span><?php _e('Constant Contact', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Constant Contact with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_9" value="9" <?php  if( isset($constant_contact_arr['enable']) and $constant_contact_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('constant', '9');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_9"><span class="ar_lbl_span"></span><?php _e('Constant Contact', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Constant Contact with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                    ...
                                    <div id="arfem_getresponse_div" class="arfemdiv" <?php if($current_active_ar == 'getresponse') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['getresponse_type'] != 2 ) { 
											$autochecked = '';
											...
											?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_4" value="4" <?php echo $autochecked;?> onchange="show_setting('getresponse', '4');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_4"><span class="ar_lbl_span"></span><?php _e('Getresponse', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Getresponse with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_4" value="4" <?php if( isset($getresponse_arr['enable']) and $getresponse_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('getresponse', '4');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_4"><span class="ar_lbl_span"></span><?php _e('Getresponse', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure Getresponse with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                    ...
                                    <div id="arfem_gvo_div" class="arfemdiv" <?php if($current_active_ar == 'gvo') { echo 'style="display:block;"'; }?>>
                                    	
                                        <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['gvo_type'] != 2 ) { 
											$autochecked = '';
											if( $res['gvo_type'] == 0 and trim($res4[0]['responder_web_form']) != '' )
												$autochecked = 'checked="checked"';	
											?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_5" value="5" <?php echo $autochecked;?> onchange="show_setting('gvo', '5');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_5"><span class="ar_lbl_span"></span><?php _e('GVO', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure GVO with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_5" value="5" <?php if($res['gvo_type'] == 2) { echo 'disabled="disabled"'; }?> <?php if( isset($gvo_arr['enable']) and $gvo_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('gvo', '5');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_5"><span class="ar_lbl_span"></span><?php _e('GVO', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure GVO with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                    ...
                                    <div id="arfem_ebizac_div" class="arfemdiv" <?php if($current_active_ar == 'ebizac') { echo 'style="display:block;"'; }?>>
                                    	
                                         <div class="arfemailcontrol">
                                        	<?php if( ($arfaction =='new' || ( $arfaction =='duplicate' and $arf_template_id < 100 ) ) and $res['ebizac_type'] != 2 ) { 
											$autochecked = '';
											if( $res['ebizac_type'] == 0 and trim($res5[0]['responder_web_form']) != '' ){
												$autochecked = 'checked="checked"';
											}
											?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_6" value="6" <?php echo $autochecked;?> onchange="show_setting('ebizac', '6');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_6"><span class="ar_lbl_span"></span><?php _e('eBizac', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure eBizac with this form.)', 'ARForms'); } ?></font></label>
        									<?php } else { ?>
                                            <input type="checkbox" class="chkstanard" name="autoresponders[]" id="autores_6" value="6" <?php if($res['ebizac_type'] == 2) { echo 'disabled="disabled"'; }?> <?php if( isset($ebizac_arr['enable']) and $ebizac_arr['enable'] == 1) { echo "checked=checked"; } ?> onchange="show_setting('ebizac', '6');" <?php if($setact != 1) { echo 'onclick="return false"'; } ?>  /><label for="autores_6"><span class="ar_lbl_span"></span><?php _e('eBizac', 'ARForms'); ?><font style="color:#FF0000; font-size:13px;"><?php if($setact != 1) { _e('&nbsp;&nbsp;&nbsp;(Please Activate your license to configure eBizac with this form.)', 'ARForms'); } ?></font></label>
                                            <?php } ?>		
                                        </div>
                                        ...
                                        <div class="arfcolumnright" style="display:table-row">
                                            ...
                                            <div class="arfcolumnleft" style="display:table-cell; width:160px">
                                                ...
                                                <span id="frm_responder_first_name">
                                                    <div class="sltstandard" style="float:none;">
                                                    <?php /*?><select name="autoresponder_fname" class="arf_name_field_dropdown" style="width:160px;" data-width="160px" data-size="10" <?php if($setact != 1) { echo "readonly=readonly"; } ?> >
                                                        <option value=""><?php _e('Select First Name', 'ARForms'); ?></option>
                                                        <?php if( isset($values['fields']) and count($values['fields']) > 0 ){ foreach($values['fields'] as $field1){ 
                                                            if( $field1['type'] != 'divider' && $field1['type'] != 'break' && $field1['type'] != 'captcha' && $field1['type'] != 'html' ){ ?>
                                                        <option value="<?php echo ($field1["ref_field_id"] > 0 ) ? $field1["ref_field_id"] : $field1["id"];?>" <?php if( ($field1["id"]==$responder_fname) || ($field1["ref_field_id"]==$responder_fname) ){?> selected="selected" <?php }?>><?php echo $field1["name"];?></option>
                                                        <?php } 
                                                        } } ?>
                                                    </select><?php */?>
                                                    ...
                                                    <input id="autoresponder_fname" name="autoresponder_fname" value="<?php echo $responder_fname;?>" type="hidden" <?php if($setact != 1) { echo "readonly=readonly"; } ?>>
                                                    <dl class="arf_selectbox" data-name="autoresponder_fname" data-id="autoresponder_fname" style="width:140px;">
                                                      <dt><span><?php if( $selectbox_field_value_label!="" ) { echo $selectbox_field_value_label; } else { echo __('Select First Name', 'ARForms'); }?></span>
                                                        <input value="<?php if( $values["id"]==$responder_fname ) { echo $values["id"]; } else if($values["ref_field_id"]==$responder_fname) { echo $values["ref_field_id"]; }?>" style="display:none;width:128px;" class="arf_autocomplete" type="text">
                                                        <i class="fa fa-caret-down fa-lg"></i></dt>
                                                      <dd>
                                            ...
                                            <div class="arfcolumnright" style="display:table-cell; width:160px">
                                                ...
                                                <span id="frm_responder_last_name">          
                                                    <div class="sltstandard" style="float:none;">
                                                    <?php /*?><select name="autoresponder_lname" class="arf_name_field_dropdown" style="width:160px;" data-width="160px" data-size="10" <?php if($setact != 1) { echo "readonly=readonly"; } ?> >
                                                        <option value=""><?php _e('Select Last Name', 'ARForms'); ?></option>
                                                        <?php if( isset($values['fields']) and count($values['fields']) > 0 ){ foreach($values['fields'] as $field1){ 
                                                            if( $field1['type'] != 'divider' && $field1['type'] != 'break' && $field1['type'] != 'captcha' && $field1['type'] != 'html' ){ ?>
                                                        <option value="<?php echo ($field1["ref_field_id"] > 0 ) ? $field1["ref_field_id"] : $field1["id"];?>" <?php if( ($field1["id"]==$responder_lname) || ($field1["ref_field_id"]==$responder_lname) ){?> selected="selected" <?php }?>><?php echo $field1["name"];?></option>
                                                        <?php } 
                                                        } } ?>
                                                    </select><?php */?>
                                                  ...
                                                  <?php 
													...
													?>
                                                        <input id="autoresponder_lname" name="autoresponder_lname" value="<?php echo $responder_lname;?>" type="hidden" <?php if($setact != 1) { echo "readonly=readonly"; } ?>>
                                                        <dl class="arf_selectbox" data-name="autoresponder_lname" data-id="autoresponder_lname" style="width:140px;">
                                                          <dt><span><?php if( $selectbox_field_value_label!="" ) { echo $selectbox_field_value_label; } else { echo __('Select Last Name', 'ARForms'); }?></span>
                                                            <input value="<?php if( $values["id"]==$responder_lname ) { echo $values["id"]; } else if($values["ref_field_id"]==$responder_lname) { echo $values["ref_field_id"]; }?>" style="display:none;width:128px;" class="arf_autocomplete" type="text">
                                                            <i class="fa fa-caret-down fa-lg"></i></dt>
                                                          <dd>
                                    ...
                                    <div class="arftablerow" style="display:table-cell;">
                                    	...
                                    	<div class="arfcolumnright">
                                              ...
                                              <span id="frm_responder_email">
                                                    <div class="sltstandard" style="float:none;">
                                                    <?php /*?><select name="autoresponder_email" class="arf_email_field_dropdown" style="width:160px;" data-width="160px" data-size="10" <?php if($setact != 1) { echo "readonly=readonly"; } ?> >
                                                    <option value=""><?php _e('Select Field', 'ARForms'); ?></option>
                                                    <?php if( isset($values['fields']) and count($values['fields']) > 0 ){ foreach($values['fields'] as $field1){ 
                                                        if(in_array($field1['type'], array('email','text'))){  ?>
                                                    <option value="<?php echo ($field1["ref_field_id"] > 0 ) ? $field1["ref_field_id"] : $field1["id"];?>" <?php if( ($field1["id"]==$responder_email) || ($field1["ref_field_id"]==$responder_email) ){?> selected="selected" <?php }?>><?php echo $field1["name"];?></option>
                                                    <?php } 
                                                    } } ?>
                                                    </select><?php */?>
											...
											?>
                                                    	...
                                                    	<input id="autoresponder_email" name="autoresponder_email" value="<?php echo $responder_email;?>" type="hidden" <?php if($setact != 1) { echo "readonly=readonly"; } ?>>
                                                        <dl class="arf_selectbox" data-name="autoresponder_email" data-id="autoresponder_email" style="width:140px;">
                                                          <dt><span><?php if( $selectbox_field_value_label!="" ) { echo $selectbox_field_value_label; } else { echo __('Select Field', 'ARForms'); }?></span>
                                                            <input value="<?php if( $values["id"]==$responder_email ) { echo $values["id"]; } else if($values["ref_field_id"]==$responder_email) { echo $values["ref_field_id"]; }?>" style="display:none;width:128px;" class="arf_autocomplete" type="text">
                                                            <i class="fa fa-caret-down fa-lg"></i></dt>
                                                          <dd>
                	...
                	<div id="arf_customcss" class="arfsettingsubcontainer" style="border:none;">
                        ...
                        <div class="arfformtable">
                            ...
                            <div class="arftablerow">
                                ...
                                <div class="arfcolumnright">
                                	<button class="arfcustomcssbtn <?php if($customcss_array['arf_form_outer_wrapper']){ echo 'arfactive'; }?>" id="arf_form_outer_wrapper_btn" onclick="add_custom_css_block('arf_form_outer_wrapper', '<?php echo addslashes(__('Form outer wrapper','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Form outer wrapper','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_inner_wrapper']){ echo 'arfactive'; }?>" id="arf_form_inner_wrapper_btn" onclick="add_custom_css_block('arf_form_inner_wrapper', '<?php echo addslashes(__('Form inner wrapper','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Form inner wrapper','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_title']){ echo 'arfactive'; }?>" id="arf_form_title_btn" onclick="add_custom_css_block('arf_form_title', '<?php echo addslashes(__('Form title','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Form title','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_description']){ echo 'arfactive'; }?>" id="arf_form_description_btn" onclick="add_custom_css_block('arf_form_description', '<?php echo addslashes(__('Form description','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Form description','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_element_wrapper']){ echo 'arfactive'; }?>" id="arf_form_element_wrapper_btn" onclick="add_custom_css_block('arf_form_element_wrapper', '<?php echo addslashes(__('Field wrapper','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Field wrapper','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_element_label']){ echo 'arfactive'; }?>" id="arf_form_element_label_btn" onclick="add_custom_css_block('arf_form_element_label', '<?php echo addslashes(__('Field label','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Field label','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_elements']){ echo 'arfactive'; }?>" id="arf_form_elements_btn" onclick="add_custom_css_block('arf_form_elements', '<?php echo addslashes(__('Input elements','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Input elements','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_page_break']){ echo 'arfactive'; }?>" id="arf_form_page_break_btn" onclick="add_custom_css_block('arf_form_page_break', '<?php echo addslashes(__('Page break','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Page break','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_submit_outer_wrapper']){ echo 'arfactive'; }?>" id="arf_submit_outer_wrapper_btn" onclick="add_custom_css_block('arf_submit_outer_wrapper', '<?php echo addslashes(__('Submit wrapper','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Submit wrapper','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_submit_button']){ echo 'arfactive'; }?>" id="arf_form_submit_button_btn" onclick="add_custom_css_block('arf_form_submit_button', '<?php echo addslashes(__('Submit button','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Submit button','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_next_button']){ echo 'arfactive'; }?>" id="arf_form_next_button_btn" onclick="add_custom_css_block('arf_form_next_button', '<?php echo addslashes(__('Next button','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Next button','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_previous_button']){ echo 'arfactive'; }?>" id="arf_form_previous_button_btn" onclick="add_custom_css_block('arf_form_previous_button', '<?php echo addslashes(__('Previous button','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Previous button','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_success_message']){ echo 'arfactive'; }?>" id="arf_form_success_message_btn" onclick="add_custom_css_block('arf_form_success_message', '<?php echo addslashes(__('Success message','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Success message','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_error_message']){ echo 'arfactive'; }?>" id="arf_form_error_message_btn" onclick="add_custom_css_block('arf_form_error_message', '<?php echo addslashes(__('Validation (Error)','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Validation (Error)','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_fly_sticky']){ echo 'arfactive'; }?>" id="arf_form_fly_sticky_btn" onclick="add_custom_css_block('arf_form_fly_sticky', '<?php echo addslashes(__('Fly / Sticky Button','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Fly / Sticky Button','ARForms');?></button>&nbsp;&nbsp;
                                    
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_modal_css']){ echo 'arfactive'; }?>" id="arf_form_modal_css_btn" onclick="add_custom_css_block('arf_form_modal_css', '<?php echo addslashes(__('Modal','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Modal','ARForms');?></button>&nbsp;&nbsp;
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_link_css']){ echo 'arfactive'; }?>" id="arf_form_link_css_btn" onclick="add_custom_css_block('arf_form_link_css', '<?php echo addslashes(__('Link (Popup)','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Link (Popup)','ARForms');?></button>&nbsp;&nbsp;
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_link_hover_css']){ echo 'arfactive'; }?>" id="arf_form_link_hover_css_btn" onclick="add_custom_css_block('arf_form_link_hover_css', '<?php echo addslashes(__('Link Hover (Popup)','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Link Hover (Popup)','ARForms');?></button>&nbsp;&nbsp;
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_button_css']){ echo 'arfactive'; }?>" id="arf_form_button_css_btn" onclick="add_custom_css_block('arf_form_button_css', '<?php echo addslashes(__('Button (Popup)','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Button (Popup)','ARForms');?></button>&nbsp;&nbsp;  
                                    <button class="arfcustomcssbtn <?php if($customcss_array['arf_form_button_hover_css']){ echo 'arfactive'; }?>" id="arf_form_button_hover_css_btn" onclick="add_custom_css_block('arf_form_button_hover_css', '<?php echo addslashes(__('Button Hover (Popup)','ARForms'));?>', '<?php echo addslashes(__('Remove','ARForms'));?>');" type="button"><?php _e('Button Hover (Popup)','ARForms');?></button>&nbsp;&nbsp;  
                                </div>
                            </div>
                            ...
                            <div class="arftablerow">
                                ...
                                <div class="arfcolumnright"><textarea id="arf_form_other_css" name="options[arf_form_other_css]" style="width:430px !important;" cols="50" rows="4" class="arfplacelonginput txtmultinew"><?php $values['arf_form_other_css'] = isset($values['arf_form_other_css']) ? $values['arf_form_other_css'] : ''; echo $armainhelper->esc_textarea($arformcontroller->br2nl($values['arf_form_other_css'])); ?></textarea>
            ...
            <div id="arfaddtosite" style="display:none;">
                ...
                <div class="arfaddtosite_sidebar">
                    ...
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $normal_view_style; ?>">[ARForms id=<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?>]</div>
                    
                    
                    <div class="arfaddtosite_sidebar_title"><?php _e('Pop-up View', 'ARForms'); ?></div>
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $popup_view_style; ?>">[ARForms_popup id=<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?> desc="Click here to open Form" type="link" height="540" width="800"]</div>
                    ...
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $fly_view_style; ?>">[ARForms_popup id=<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?> desc="Click here to open Form" type="fly" position="left" height="540" width="800"]</div>
                    ...
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $sticky_view_style; ?>" >[ARForms_popup id=<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?> desc="Click here to open Form" type="sticky" position="top" height="540" width="800"]</div>
                    ...
                    <div class="arfaddtosite_sidebar_title"><?php _e('Normal View', 'ARForms'); ?></div>
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $normal_view_style_php; ?>">&lt;?php echo maincontroller::get_form_shortcode(array('id' => '<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?>')); ?></div>
                    
                    <div class="arfaddtosite_sidebar_title"><?php _e('Pop-up View', 'ARForms'); ?></div>
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $popup_view_style_php; ?>">&lt;?php echo maincontroller::get_form_shortcode_popup(array('id' => '<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?>', 'desc'=>'Click here to open Form', 'type'=>'link', 'height'=>'540', 'width'=>'800')); ?></div>
                    
                    <div class="arfaddtosite_sidebar_title"><?php _e('Fly View', 'ARForms'); ?></div>
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $popup_view_style_php; ?>">&lt;?php echo maincontroller::get_form_shortcode_popup(array('id' => '<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?>', 'desc'=>'Click here to open Form', 'type'=>'fly', 'position'=>'left', 'height'=>'540', 'width'=>'800')); ?></div>
                    
                    <div class="arfaddtosite_sidebar_title"><?php _e('Sticky view', 'ARForms'); ?></div>
                    <div class="arfaddtosite_sidebar_subtitle" style=" <?php echo $popup_view_style_php; ?>">&lt;?php echo maincontroller::get_form_shortcode_popup(array('id' => '<?php if($ref_formkey!=0) { echo $ref_formkey; } else { echo $id; } ?>', 'desc'=>'Click here to open Form', 'type'=>'sticky', 'position'=>'top', 'height'=>'540', 'width'=>'800')); ?></div>
