...
<?php if(isset($is_create_new_field) && $is_create_new_field == 1) { require(VIEWS_PATH.'/addfieldjs.php'); } ?>
...
?>
    ...
    <div class="fieldname">
		...
		<?php if ($display['required'] and $field['type'] != 'slider' && $field['type'] != 'imagecontrol'){ ?>
...
<div class="arf_fieldiconbox">
    <div class="field-icon arfshowiconsonhover<?php echo $field['id']; ?>" style="visibility:hidden;">
        ...
        <a href="javascript:void(0);" onclick="arfshowfieldoptions('<?php echo $field['id']; ?>')" id="field-setting-button-<?php echo $field['id']; ?>" class="frm-setting arfhelptip" title="<?php _e('Settings', 'ARForms'); ?>" data-lower="<?php if( version_compare( $GLOBALS['wp_version'], '3.3', '<') ){echo "true";}else{ echo "false";}?>"><img id="field-setting-icon<?php echo $field['id']; ?>" src="<?php echo ARFIMAGESURL ?>/editor_icons/setting-icon_hover.png" alt="Duplicate" /></a> 
        </div>
        <?php } ?>
...
<?php 
    ...
    <div class="arf_checkbox_div" id="arf_checkbox_div_<?php echo $field['id']; ?>">
    	<div id="arf_checkboxradio_<?php echo $field['id']; ?>" class="arf_checkboxradio_inside <?php if($field['align'] == 'inline') { echo 'arf_single_row'; } else { echo 'arf_multiple_row';}?>">
        <?php
        ...
        <div id="arffieldoptions_<?php echo $field['id'] ?>" class="show-field-options arfmodal" style="display:none;margin-top:0px;top:0px;position:absolute;width:auto;min-width: 200px;">
            ...
            <div class="arfmodal-body_fieldoptions">
                ...
                <div id="frm_add_field_<?php echo $field['id']; ?>" class="arfshowfieldclick"><br  />
                    ...
                    <button onclick="javascript:arfaddnewfieldoption(<?php echo $field['id']; ?>)" id="arfaddanoption_<?php echo $field['id']; ?>" class="btn_2" type="button" style="height:26px;font-size:12px;margin:0px 5px 5px 0px;line-height:23px;padding:0 10px;<?php if( ! is_array($field['options']) || ! ($field['options']) ){ echo 'display:inline;'; } else { echo 'display:none;';} ?>"><?php _e('Add an Option', 'ARForms') ?></button>
                    ...
                    <button onClick="arfupdatebulkoptions(<?php echo $field['id'] ?>,jQuery('#frm_bulk_options_sel-<?php echo $field['id'];?>').val()); return false;" class="btn_apply" style="margin-right:5px;"><?php _e('Apply', 'ARForms') ?></button>
                    </div>
...
if ($display['options']){ ?>
        ...
        <div class="fieldoptions_tab_main">
            ...
            <div id="field_custom_css_tab" class="fieldoption_inner_tab" onclick="arf_open_field_option('<?php echo $field['id']; ?>', 'field_custom_css','0');" style=" <?php if( ($field['is_recaptcha'] == 'recaptcha' and $field['type'] == 'captcha' ) || $field['type'] == 'break' || $field['type'] == 'imagecontrol' ){ echo 'display:none;'; }?>"><?php _e('CSS Property', 'ARForms') ?></div>
            <div style=" <?php if( $field['type'] == 'imagecontrol' ){ echo 'display:none;'; }?>" id="field_conditional_law_tab" class="fieldoption_inner_tab" onclick="arf_open_field_option('<?php echo $field['id']; ?>', 'field_conditional_law','0');"><?php _e('Conditional Law', 'ARForms') ?></div>
        </div>
					   ...
					   if($arffieldoption == $field['type'])
					   {
							...
							if($totaloptions > 0)
							{
								...
								{ 
									...
									<?php if($int%3 == 0 && $totaloptions > $int)
									{ 
										$innercounter = 0;
										echo "</tr>";
            ...
            <div class="field_css">
                <div style="margin-top:20px;">
                    ...
                    <div style="margin-left:100px;">
						...
						foreach($custom_css_array as $custom_css_block => $custom_css_block_title){
							?>
							<button class="arfcustomcssbtn <?php if($customcss_array[ $custom_css_block ]){ echo 'arfactive'; }?>" id="<?php echo $custom_css_block.'_'.$field['id'];?>_btn" onclick="add_fieldcustom_css_block('<?php echo $field['id']?>', '<?php echo $custom_css_block;?>', '<?php echo addslashes($custom_css_block_title);?>');" type="button"><?php echo $custom_css_block_title;?></button>&nbsp;&nbsp;
							<?php
						}
		...
		<div style=" <?php echo isset($padding) ? $padding : '';?> padding-top:10px; width:90%;">
				...
				<div id="conditional_logic_div_<?php echo $field['id'] ?>" style="width:auto; margin-bottom:20px; font-size:14px;">
					...
					<div class="sltstandard<?php if( count($cl_rules_array) == 0 ){ echo ' arfhelptip'; }?>" <?php if( count($cl_rules_array) == 0 ){ ?>title="<?php _e('Please add one or more rules', 'ARForms'); ?>"<?php }?> style="float:none;display:inline-block;margin-right:15px;">
                    ...
                    <input id="conditional_logic_display_<?php echo $field['id']; ?>" name="conditional_logic_display_<?php echo $field['id']; ?>" value="<?php if($field['conditional_logic']['display']=='hide') { echo 'hide'; } else { echo 'show'; } ?>" type="hidden" <?php if( count($cl_rules_array) == 0 ){ ?>disabled="disabled"<?php }?>>
                      <dl class="arf_selectbox" data-name="conditional_logic_display_<?php echo $field['id']; ?>" data-id="conditional_logic_display_<?php echo $field['id']; ?>" style="width:60px;">
                        <dt class="<?php if( count($cl_rules_array) == 0 ){ echo "arf_disable_selectbox"; }?>"><span><?php if($field['conditional_logic']['display']=='hide') { echo __('Hide', 'ARForms'); } else { echo __('Show', 'ARForms'); } ?></span>
                        <input value="<?php if($field['conditional_logic']['display']=='hide') { echo __('Hide', 'ARForms'); } else { echo __('Show', 'ARForms'); } ?>" style="display:none;width:48px;" class="arf_autocomplete" type="text">
                        <i class="fa fa-caret-down fa-lg"></i></dt>
                        <dd>
                    ...
                    <input id="conditional_logic_if_cond_<?php echo $field['id']; ?>" name="conditional_logic_if_cond_<?php echo $field['id']; ?>" value="<?php if($field['conditional_logic']['if_cond']=='any') { echo 'any'; } else { echo 'all'; } ?>" type="hidden" <?php if( count($cl_rules_array) == 0 ){ ?>disabled="disabled"<?php }?>>
                      <dl class="arf_selectbox" data-name="conditional_logic_if_cond_<?php echo $field['id']; ?>" data-id="conditional_logic_if_cond_<?php echo $field['id']; ?>" style="width:60px;">
                        <dt class="<?php if( count($cl_rules_array) == 0 ){ echo "arf_disable_selectbox"; }?>"><span><?php if($field['conditional_logic']['if_cond']=='any') { echo __('Any', 'ARForms'); } else { echo __('All', 'ARForms'); } ?></span>
                        <input value="<?php if($field['conditional_logic']['if_cond']=='any') { echo __('Any', 'ARForms'); } else { echo __('All', 'ARForms'); } ?>" style="display:none;width:48px;" class="arf_autocomplete" type="text">
                        <i class="fa fa-caret-down fa-lg"></i></dt>
                        <dd>
					...
					<!-- logic rules start -->
                    <button type="button" id="arf_new_law_<?php echo $field['id']; ?>" onclick="arf_add_new_law('<?php echo $field['id']; ?>');" style=" <?php if( $field['conditional_logic']['enable'] == 1 ) { echo 'display:none;'; } ?>" class="greensavebtn arfaddnewrule"><?php _e('Add New Law', 'ARForms'); ?></button>
                    
					<div id="logic_rules_div_<?php echo $field['id']; ?>" style=" <?php if( $field['conditional_logic']['enable'] == 0 ) { echo 'display:none;'; }else { echo "display:inline-block;"; } ?>" class="logic_rules_div">
						<?php 
