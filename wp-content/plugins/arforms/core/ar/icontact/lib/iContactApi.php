...
class iContactApi {
	...
	**/
	public function makeCall($sResource, $sMethod = 'GET', $mPostData = null, $sReturnKey = null) {
		...
		$sResource = str_replace('//', '/',$sResource);		
		...
		switch (strtoupper($sMethod)) {
			...
			case 'GET'    :
				...
				$temp_header4 = @explode(':', $headers[3]);
				$temp_header5 = @explode(':', $headers[4]);
				$temp_header6 = @explode(':', $headers[5]);
				$temp_header7 = @explode(':', $headers[6]);
			
				$new_header = array($temp_header1[0] => $temp_header1[1], $temp_header2[0]=>$temp_header2[1], $temp_header3[0]=>$temp_header3[1], $temp_header4[0]=>$temp_header4[1], $temp_header5[0]=>$temp_header5[1], $temp_header6[0]=>$temp_header6[1], $temp_header7[0]=>$temp_header7[1] );	
	
				$temp_arr = $this->aSearchParameters;
				
				$temp_arr['sslverify'] 		= false;				
				$temp_arr['blocking']		= true;
				$temp_arr['redirection'] 	= 5;
				$temp_arr['httpversion'] 	= '1.0';
				$temp_arr['timeout']		= 90;
				$temp_arr['blocking']		= true;
				$temp_arr['headers']		= @$new_header;
				$temp_arr['method'] 		= 'GET';
						
				$sResponse = wp_remote_get( $sApiUrl, $temp_arr );
			...
			case 'POST'   :
				...
				$sResponse = wp_remote_post( $sApiUrl, array(
								'method' => 'POST',
								'timeout' => 45,
								'redirection' => 5,
								'httpversion' => '1.0',
		...
		$this->sLastResponse = (string) json_encode($sResponse);
	
		// Try to decode the response
		if ((!$aResponse = json_decode(json_encode($sResponse))) && (strtoupper($sMethod) != 'DELETE')) {
		...
		$tmp = json_decode($aResponse->body);
		$aResponse->$sReturnKey = $tmp->$sReturnKey;
