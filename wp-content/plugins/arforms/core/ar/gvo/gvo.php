<?php
	$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix . "arf_autoresponder WHERE responder_id='5'");$res = $res[0];
	
	$data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM ".$wpdb->prefix."arf_ar WHERE frm_id = %d", $fid), 'ARRAY_A' );
			
   	$arr_gvo 	= unserialize( $data[0]['gvo'] );
	
	$responder_api_key = @stripslashes( stripslashes_deep( $arr_gvo['type_val'] ) );
	...
	if(in_array("FullName",$formdata_n))
	{
