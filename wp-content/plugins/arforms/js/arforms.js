jQuery(document).ready(function($)
{
	...
	var trigger=$('.arfblankfield').closest('.toggle_container').prev('.trigger_style');if(trigger)arfsectionswap(trigger);
					...
					if( total_error > 0 )
					{
						...
						setTimeout(function(){																		
							...
							if( focus_field_id != '' && focus_error_msg != '' )
							{
								...
								for(k2 in msgObj)
								{
									if(k2 == fieldname) focus_error_msg = msgObj[k2];							
								}
			...
			if(conf_method != "")
			{
				...
				else if(conf_method == 'message')
				{
					if(document.getElementById('popup-form-'+form_id))
					{
						...
						setTimeout(function(){ jQuery('#popup-form-'+form_id).find('#arf_message_success').parent().hide("slow");},6000); 	
					...
					else
					{
						...
						setTimeout(function(){jQuery('#arffrm_'+ob_form_id+'_container #arf_message_success').parent().hide("slow");},6000);	 
					}
				}
				else if(conf_method == 'page')
				{
					if(document.getElementById('popup-form-'+form_id))
					{
						...
						setTimeout(function(){ jQuery('#popup-form-'+form_id).find('#arf_message_success').hide("slow");},6000); 
					}
					else
					{
						...
						if( jQuery(msg).find('.recaptcha_style_custom').length > 0 )
						{
							...
							setTimeout(function(){jQuery('#form_'+form_key).find("#recaptcha_style").append('<div id="recaptcha_widget_div"><div id="recaptcha_area"></div></div>');
							Recaptcha.widget = Recaptcha.$("recaptcha_widget_div"); 
							Recaptcha.challenge_callback();
							/*Recaptcha.reload("t");*/ },1000);	
						}
						setTimeout(function(){jQuery('#arf_message_success').hide("slow");},6000);	
					}
				}
