...
$.fn.editInPlace.defaults = {
	...
	bg_over:			"#BFE9F3", // string: background color of hover of unactivated editor
	...
	saving_text:		"Saving...", // string: text to be used when server is saving information. Example "Saving..."
...
$.extend(InlineEditor.prototype, {
	...
	addHoverEffect: function() {
		if (this.settings.hover_class)
			...
			var fi_id = this.dom.attr("id");
			if( fi_id != '' && fi_id !== undefined ){
					fi_id = fi_id.replace('field_', ''); 
			}
			//jQuery('#arfmainfieldid_'+fi_id+' .fieldname').css('width', '60%');
			jQuery('#arfmainfieldid_'+fi_id+' .fieldname').css("background-color", this.settings.bg_over);
			//this.dom.css("padding", '3px');
	},
	...
	removeHoverEffect: function() {
		if (this.settings.hover_class)
			...
			//jQuery('.fieldname').css('width', '');
			jQuery('.fieldname').css("background-color", this.settings.bg_out);
			//this.dom.css("padding", '0px');
			//jQuery('.fieldname').css("padding-bottom", '10px');
	},
	...
	removeInsertedDefaultTextIfNeccessary: function() {
		...
		if ( this.dom.html() !== this.settings.default_text) 
			return;
	...
	setClosedEditorContent: function(aValue) {
		...
		else
		{
			if(aValue!="")
				...
				this.dom.text(this.settings.default_text);
		}
	},
	...
	replaceContentWithEditor: function() {
		...
		//this.dom.html('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;">'+ editorElement +'</form>');
		//this.dom.html('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;">'+ editorElement +'</form>');
		//var searchform = $("form.inplace_form")
		//this.dom.html('').append('<form id="inplace_form" class="inplace_form" style="display: inline; margin: 0; padding: 0;"></form>')
						//.find("form")
		...
		var from=$('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;"></form>');
				...
				.append(buttons_html);
		//this.dom.html('<form id="inplace_form" class="inplace_form" style="display: inline; margin: 0; padding: 0;"></form>');
			//this.dom.find("form[id!='frm_main_form']")
			//this.dom.find("form > form")
			//this.dom
	},
	...
	createEditorElement: function() {
		...
		if( classname.indexOf('arfeditorformname') >= 0 )
		{
			jQuery('.arfformeditpencil').hide();
			jQuery('.arfformedit').css('width', '98.7%');
		}
		if( classname.indexOf('arfeditorformdescription') >= 0 )
		{
			jQuery('.arfdescriptioneditpencil').hide();
			jQuery('.arfdescriptionedit').css('width', '98.7%');
		}
		if( classname.indexOf('arfsubmitbtn') >= 0 )
		{
			jQuery('.arfsubmiteditpencil').hide();
			jQuery('.arfsubmitsettingpencil').hide();
			jQuery('.arfsubmitedit .greensavebtn').css('padding','5px');
			
			var s_width = jQuery('.arfsubmitedit .greensavebtn').width();
			var data_auto = jQuery('.arfsubmitedit .greensavebtn').attr('data-auto');
			if( data_auto == 0 )
			{
				jQuery('.arfsubmitedit .greensavebtn').attr('data-width', s_width);
				jQuery('.arfsubmitedit .greensavebtn').css('width', 'auto');
			} 
			var submit_outer = jQuery('.arfsubmitedit .greensavebtn').outerWidth();
			
			if( data_auto == 1 )
				submit_outer = s_width;
				
			jQuery('#arfeditorsubmit').css('width', submit_outer+'px');
		}
	...
	/*createEditorElement: function() {
		...
		{
			editor = '<input type="text" ' + this.inputNameAndClass() + ' size="' + this.settings.text_size  + '" />';
			//editor = $('<input type="text" ' + this.inputNameAndClass() 
				//+ ' size="' + this.settings.text_size  + '" />');
		}
		else if ("textarea" === this.settings.field_type)
		{
			editor = '<textarea ' + this.inputNameAndClass() + ' rows="' + this.settings.textarea_rows + '" ' + ' cols="' + this.settings.textarea_cols + '" />';
			//editor = $('<textarea ' + this.inputNameAndClass() 
				//+ ' rows="' + this.settings.textarea_rows + '" '
				//+ ' cols="' + this.settings.textarea_cols + '" />');
		}
	...
	connectClosingEventsToEditor: function() {
		...
		//var form = this.dom.find("form > form");
	...
	bindSubmitOnEnterInInput: function() {
		...
		this.dom.find(':input').keyup(function(event) {
			...
			{
				//return that.dom.find("form > form").submit();		
	...
	handleCancelEditor: function(anEvent) {
		...
		if( classname.indexOf('arfeditorformname') >= 0 )
		{
			jQuery('.arfformeditpencil').show();
			jQuery('.arfformedit').css('width', 'auto');
		}
		if( classname.indexOf('arfeditorformdescription') >= 0 )
		{
			jQuery('.arfdescriptioneditpencil').show();
			jQuery('.arfdescriptionedit').css('width', 'auto');
		}
		if( classname.indexOf('arfsubmitbtn') >= 0 )
		{
			jQuery('.arfsubmiteditpencil').show();
			jQuery('.arfsubmitsettingpencil').show();
			jQuery('.arfsubmitedit .greensavebtn').css('padding','15px 10px 0 10px');
			var s_width 	= jQuery('.arfsubmitedit .greensavebtn').attr('data-width');
			var data_auto 	= jQuery('.arfsubmitedit .greensavebtn').attr('data-auto');
			if( data_auto == 1 )
				jQuery('.arfsubmitedit .greensavebtn').css('width', s_width+'px');
			
			jQuery('#arfeditorsubmit').removeAttr('style');
	...
	handleSaveEditor: function(anEvent) {
		...
		//alert(fi_id);
		jQuery('#arfname_'+fi_id).val(enteredText);
		...
		if( classname.indexOf('arfsubmitbtn') >= 0 )
		{
			jQuery('#arfsubmitbuttontext').val( enteredText );
		}
		//alert(jQuery('#arfname_'+fi_id).val());
		...
		if(document.getElementById('required_form_title'))
			required_form_title = document.getElementById('required_form_title').value;
		else
			required_form_title = 'Error: You must enter a value to save this field';
		
		var f_id = this.dom.attr("id");
		if( f_id != '' && f_id !== undefined ){
				f_id = f_id.replace('field_', ''); 
			var type = jQuery('#field_type_'+f_id).val();
			
			if(type != 'divider'){
				
				if (this.didForgetRequiredText(enteredText)) {
					this.handleCancelEditor(anEvent);
					this.reportError(required_form_title);
					return;
				}
				
				var o_element_id = this.dom.attr("id");
				jQuery('#'+o_element_id).attr('data-original', this.originalValue);
			}
		}
	...
	isUnchangedInput: function(enteredText) {
		var class_name = this.dom.attr("class"); 
		
		if( class_name.indexOf('arfoptioneditorfield') >= 0 || class_name.indexOf('arfeditorformname') >= 0 || class_name.indexOf('arfeditorformdescription') >= 0 )
		{
		...
		/*else if( ( class_name != "arfeditorformname editInPlace-active" ) && ( class_name != "arfeditorformdescription editInPlace-active" ) )
		{	
			this.originalValue = enteredText;
		}*/
