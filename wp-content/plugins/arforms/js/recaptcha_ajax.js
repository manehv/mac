function checkRecaptcha(object, ajax_url, next_id, max_id, break_val, is_preview){
	
	var recaptcha_challenge_field = jQuery(object).find('input[name="recaptcha_challenge_field"]').attr('value');
	
	var recaptcha_response_field = jQuery(object).find('input[name="recaptcha_response_field"]').attr('value');	
	
	var form_id = jQuery(object).find('input[name="form_id"]').attr('value');	
	
	var submit_type = jQuery(object).find('#form_submit_type').val();
	
	jQuery(object).find('.arfmainformfield').removeClass('arfblankfield'); 

	jQuery(object).find('.arfmainformfield .help-block').empty(); 
	
	var vpb_captcha_code = jQuery('#vpb_captcha_code_'+form_id).val();
	
	ajax_url = is_ssl_replace(ajax_url);
	
	jQuery.ajax({type:"POST",url:ajax_url,data:"arfaction=check_recaptcha&controller=entries&plugin=ARForms&_ajax_nonce=1&recaptcha_challenge_field="+recaptcha_challenge_field+"&recaptcha_response_field="+recaptcha_response_field+"&form_id="+form_id+"&vpb_captcha_code="+vpb_captcha_code, 
			success:function(msgObj){
				
					msgObj=msgObj.replace(/^\s+|\s+$/g,'');
					if(msgObj.indexOf('{') === 0)
						var msgObj=jQuery.parseJSON(msgObj);
					
					for (var key in msgObj){
						if(key != '' && msgObj[key] != '') {
							
							if( msgObj[key] == 'success' ){
								//return true;
								
								jQuery(object).find('#recaptcha_style .popover').remove();
								jQuery(object).find('#recaptcha_style').parents('.arfformfield').find('.popover').remove();
								...
								if( break_val == 1 ){
									...
									if(is_preview == 'yes') {
											...
											setTimeout(function(){
												jQuery('#form_success_'+form_id).show();
												jQuery('html, body').animate({ scrollTop:jQuery('#form_success_'+form_id)}, 'slow' ); 
												jQuery('#form_'+form_key).show(); 
												jQuery(object).find('input[type="submit"]').removeAttr('style'); jQuery(object).find('div').removeClass('arfblankfield'); 
											...
											if( is_formreset == 1 )
											{
												jQuery('#form_'+form_key).trigger("reset");
												...
												jQuery(object).find('.original_normal').each(function(){
													...
													jQuery('#file_name_'+field_key).text('No file selected');	
												});
											...
											if( is_formreset_outside == 1 )
											{
												...
												if( jQuery(object).find( '.page_break' ).length > 1 )
													go_previous('0', form_id, 'yes', form_key);
													
											
												if(jQuery(object).find('#vpb_captcha_code_'+form_id).length > 0)
												{
													jQuery(object).find("#vpb_captcha_code_"+form_id).val('');
													jQuery(object).find("#vpb_captcha_"+form_id).addClass('control-group');
													jQuery(object).find('#captchaimg_'+form_id).attr('src',jQuery(object).find('#captchaimg_'+form_id).attr('src').substring(0,jQuery(object).find('#captchaimg_'+form_id).attr('src').lastIndexOf("?"))+"?rand="+Math.random()*1000+"&form_id="+form_id);							
												}
												else
												{
											...
											setTimeout(function(){jQuery('#form_success_'+form_id).hide("slow");},6000);
										
									} else {
										
										
											
										if(jQuery('#form_submit_type').val() == 0 && jQuery(object).find('#vpb_captcha_code_'+form_id).length > 0)
										{
											if( jQuery(object).find('#recaptcha_style').length > 0 )
												jQuery(object).find('#recaptcha_style').html('  '); 
										}
										
										if((jQuery('#form_submit_type').val() == 0 || jQuery('#form_submit_type').val() == 1) && jQuery(object).find('#vpb_captcha_code_'+form_id).length <= 0)
										{
											if( jQuery(object).find('#recaptcha_style').length > 0 )
												jQuery(object).find('#recaptcha_style').html('  '); 
										}
										...
										if( submit_type == 1 ){		
											...
											jQuery(object).find('.arf_submit_btn').attr('disabled', true);
											setTimeout(function(){
												arfformsubmission(object,ajax_url, 'yes');
											}, 2000);
										} else {		
											...
											if( is_submit_form == 1 ){
												if( jQuery(object).find('#recaptcha_style').length > 0 )
													jQuery(object).find('#recaptcha_style').remove(); 
								...
								if(jQuery(object).find('#arf_field_'+key+'_container #recaptcha_area').length){
									...
									if( error_type == 'advance' )
									{
										...
										arf_show_tooltip($controlGroup, $helpBlock, msgObj[key]);
										...
										if(!$helpBlock.length) {
											$helpBlock = jQuery('<div class="help-block"><ul><li>'+msgObj[key]+'</li></ul></div>');
										...
										{
											$helpBlock = jQuery('<ul role="alert"><li>'+msgObj[key]+'</li></ul>');
...
function checkRecaptcha_new(object,ajax_url){
