...
//		print_r($data);
	...
	public function saveStates($data){
		global $wpdb;
		 $sql = $wpdb->prepare("select state_code, state_name  from  ". $wpdb->prefix .
		 "states where state_name='%s' ", array($data[self::iSTATE]));
  ...
  public static function listData( $per_page = 20, $page_number = 1 ) {
		global $wpdb;
