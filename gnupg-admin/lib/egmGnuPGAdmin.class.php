 ...
 * @version   $Id: egmGnuPGAdmin.class.php,v 1.0.27 2014-11-18 21:10:00-05 egarcia Exp $
 */
...
class egmGnuPGAdmin
{
	...
	{
		print '<hr>'
			. '<footer><div class="pull-left">Versión ' . self::VERSION . '<br>Copyright &copy; 2014 EGM Ingeniería sin fronteras S.A.S.</div>'
	...
	/**
	 * Orderna un arreglo asociativo por uno de sus elementos
	 * @param array $array
	 * @param string $key
	 * @param boolean $reverse
	 * @return array
	 */
	private function sortArrayByKey($array, $key, $reverse = false)
	{
		$hash = array();
	...
	private function listKeys()
	{
		...
		// agrega el soporte de tablas con json
		$this->jsFiles[] = '//cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.3.0/bootstrap-table.min.js';
		$this->jsFiles[] = '//cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.3.0/locale/bootstrap-table-es-AR.min.js';
		...
		if ($keys === false)
			...
			print '<table id="table-keys" class="table table-striped table-hover">'
				. '<thead><tr>'
					. '<th width="10%" data-field="KeyID" data-formatter="keyidFormatter">ID</th>'
					. '<th width="60%" data-field="UserID" data-sortable="true">Nombre</th>'
					. '<th width="10%" data-field="Creation" data-sortable="true" data-halign="center" data-align="center">Creación</th>'
					. '<th width="10%" data-field="Expiration" data-sortable="true" data-halign="center" data-align="center">Vencimiento</th>'
					. '<th data-field="KeyID" data-halign="center" data-align="center" data-formatter="actionFormatter">Acciones</th>'
				. '</tr></thead>';
	
			if (empty($keys))
				print '<tbody><tr><td colspan="4">No hay llaves en el anillo.</td></tr></tbody>';
			else {
				// realiza una reduccion y formateado de la lista de llaves
				$keysJSON = array(); 
				foreach ($keys as $key) {
					$keysJSON[] = array(
						'KeyID' => $key['KeyID'],
						'UserID' => empty($key['UserID']) ? '': htmlentities($key['UserID']),
						'Creation' => substr($key['CreationDate'], 0, 10),
						'Expiration' => empty($key['ExpirationDate']) ? '': substr($key['ExpirationDate'], 0, 10)
					);
				}
				unset($keys);

				// genera la información de las llaves como un JSON
				$this->jsInline .= 'var getKeysData = function() { return ' . json_encode($keysJSON) . '; };' . PHP_EOL
					. 'var $table = $("#table-keys").bootstrapTable({'
						. ' data: getKeysData(),'
						. ' striped: true,'
						. ' pagination: true,'
						. ' pageSize: 10,'
						...
						. ' search: true'
					. '});' . PHP_EOL
					. 'function keyidFormatter(value, row) {'
						. ' var url = "' . $this->getUrl('view', array('keyID' => 'KEYID')) . '";' . PHP_EOL
						. ' return "<a href=\"" + url.replace("KEYID", value) + "\">" + value + "</a>";' . PHP_EOL
					. '}' . PHP_EOL
					. 'function actionFormatter(value, row) {'
						. ' return "<a href=\"javascript:void(deleteKey(\'" + value + "\'))\"><span class=\"glyphicon glyphicon-trash\"></span></a>";' . PHP_EOL
					. '}' . PHP_EOL;
			}
			print '</table>';
	...
	private function viewKey($keyID)
	{
		...
		print '<ol class="breadcrumb"><li><a href="' . $this->getUrl() . '">Inicio</a></li><li class="active">Información de llave</li></ol>'
			...
			. '<tr><td>Nombre</td><td>' .  htmlentities($key['UserID']) . '</td></tr>'
	...
	private function genKey()
	{
		...
		if (!empty($_POST['info'])) {
			...
			$days = empty($_POST['info']['days']) ? '0': $_POST['info']['days'];
			...
			if (!filter_var($days, FILTER_VALIDATE_INT, array(
						'options' => array(
							'default' => 0,
							'min_range' => 0
						),
				...
				$error_message .= 'Debe indicar el número de días de vigencia de la llave, 0 significa indefinido.' . PHP_EOL;
			...
			if (empty($error_message)) {
				$gpg = new egmGnuPG($this->programPath, $this->homeDirectory);
				$key = $gpg->GenKey(utf8_decode($name), utf8_decode($comment), utf8_decode($email), $password, (int)$days, 'DSA', 2048, 'ELG-E', 2048);
				if ($key) {
					// redirige a la llave
		...
		print '<ol class="breadcrumb"><li><a href="' . $this->getUrl() . '">Inicio</a></li><li class="active">Creación de llave</li></ol>'
			...
			. '<form class="form-horizontal" role="form" id="form-genkey" action="' . $this->getUrl('genkey') . '" method="post">'
				...
				. '</div>'
				. '<div class="form-group">'
					. '<label for="info_days" class="col-sm-2 control-label">Días vigencia</label>'
					. '<div class="col-sm-2">'
						. '<input type="number" class="form-control" id="info_days" name="info[days]" value="' . htmlspecialchars($days) . '" required data-bv-notempty="true" data-bv-between="true" data-bv-between-min="0" data-bv-between-max="3652">' 
					. '<span id="info_days_calculated">' . (empty($days) ? 'Sin vencimiento': date('l, M j, Y', time() + 86400 * (int)$days))  . '</span>'
					. '</div>'
				. '</div>'
				. '<div class="form-group">'
		...
		$this->jsInline = '$(document).ready(function() {
				...
				var dayNames = new Array("Domingo", "Lunes", "Martes", "Miércoles", "Jueves",
					"Viernes", "Sábado");
				$("#info_days").change(function() {
					var days = parseInt($("#info_days").val());
					if (isNaN(days)) {
						$("#info_days_calculated").html("");
					} else if (days == 0) {
						$("#info_days_calculated").html("Sin vencimiento");
					} else {
						var newdate = new Date();
						newdate.setDate(newdate.getDate() + days);
						$("#info_days_calculated").html(dayNames[newdate.getDay()] + ", " + monthNames[newdate.getMonth()] + " " + newdate.getDate() + ", " + newdate.getFullYear());
					}
				});
			});';
	}
