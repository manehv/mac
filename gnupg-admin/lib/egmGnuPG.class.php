 ...
 * @copyright Copyright (c) 2004-2014, EGM Ingenieria sin fronteras S.A.S.
 * @license   http://opensource.org/licenses/gpl-license.php GNU Public License
 * @since     Viernes, Enero 30, 2004
 * @version   $Id: gnuPG.class.php,v 1.0.18 2014-11-19 10:11:00-05 ingenieria Exp $
 */
class egmGnuPG
{
	...
	*/
	public function ListKeys($KeyKind = 'public', $SearchCriteria = '')
	{
		...
		if ( $this->_fork_process($this->programPath . ' --homedir ' . $this->homeDirectory
				. ' --with-colons --with-fingerprint ' . (($KeyKind == 'pub') ? '--list-public-keys': '--list-secret-keys')
			...
			foreach ($contents as $data) {
				...
				if (($fields[0] == 'pub') || ($fields[0] == 'sec')) {
					if (!empty($fields[5]) && preg_match('/^\d+$/', $fields[5]))
						$fields[5] = date('Y-m-d H:i:s', $fields[5]);
					if (!empty($fields[6]) && preg_match('/^\d+$/', $fields[6]))
						$fields[6] = date('Y-m-d H:i:s', $fields[6]);
					array_push($returned_keys, array(
						...
						'LocalID' => $fields[7],
						'Ownertrust' => $fields[8],
						'UserID' => $this->fixEncoding($fields[9]) 
						)
					);
					$keyPos++;
				} elseif (($fields[0] == 'uid') && ($keyPos != -1)) {
					if (empty($returned_keys[$keyPos]['UserID']))
						$returned_keys[$keyPos]['UserID'] = $this->fixEncoding($fields[9]);
				} elseif (($fields[0] == 'fpr') && ($keyPos != -1)) {
					if (empty($returned_keys[$keyPos]['Fingerprint']))
